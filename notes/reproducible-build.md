...
| L2-6 | `#1` from `.version`, which kbuild increments per link | 🟢 **MEASURED 2026-09-01 (`R4-0`, twenty-third segment), and it costs two bytes.** A fresh cell links once and `.version` reads **1**; the same tree after two `--keep` builds reads **3**. `repdiff` between the two `vmlinux`: **2 differing bytes of 3,968,240 (0.000050 %)**, at `linux_banner+0x3a` in `.rodata` and `init_uts_ns+0xc8` in `.data`, both the digit of `#1` against `#3`. So the row's claim is unchanged — `--keep` does break byte-identity — and its **cost is now a number rather than a worry**, which is what `R4-3` needed in order to decide against it. ⚠️ The bound *"starts absent and reaches 1"* held: every measurement in this file was taken on a re-staged cell. `notes/dev-loop.md` §5.1 🔄 **2026-09-02 (`R4-3`): two bytes is the cost of a DIGIT, not of the counter, and as a bound it is wrong from the tenth link on.** 量, a ladder of consecutive links from one tree: `#7→#8`, `#8→#9` and `#10→#11` each differ in **2** bytes; **`#9→#10` differs in 56**, and `#1→#12` in 54, both adding a `.symtab` byte to the two. The counter is printed into `UTS_VERSION`, which sits in `linux_banner` and in `init_uts_ns`; when its decimal rendering grows a digit the rest of both strings shifts. Nothing here is codegen. **What is corrected is the sentence, not the claim** — *"it costs two bytes"* reads as a bound and is a reading, and this row published it as if the first were true. `notes/incremental-build.md` §5.5 |
