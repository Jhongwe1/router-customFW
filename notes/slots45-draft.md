...
|---|---|---|
| 4 | `SPEC.md:641` (`FW-65` 殘留), 🔄 **was `:625`, which is a different, already-closed row — the citation was measured before the commit that shipped it added three lines above it** | `/proc/gpio` is the **vendor's** user-writable path and `FW-65` only read the compiled code. Two questions: ① does `echo 1 > /proc/gpio` get seen by `rtl819x-gpio`'s `n_state_foreign` — which would be that detector's first **non-synthetic** positive control; ② is `AutoCfg_LED_Blink` (set by `echo 2`) consumed by the second bit-6 block inside `rtl_gpio_timer`, which only runs while the timer is alive |
