...
| **Active step** | `DAY-ZERO` item 5 = `S0b` — **desk prep done, waiting for the bench.** `notes/power-and-programmer.md` Part 1 is written; Part 2 is empty by design. Items 0, 1, 2a, 2b, 2c, 3, 4 closed. `S0a` closed **with a recorded deviation** — C-10 |
| **Last session** | 2026-08-23, desk only — item 4, then item 5's prep. The loader **scans** six 64 KiB offsets for the kernel (C-1 closed); the image check is located in this unit's own code and is silent on failure (C-4's structural half closed); `EB`/`EW`/`FLR`/TFTP are four unbounded RAM-write paths and `J BFC00000` is a stock watchdog reset, so R4's `bench-ci` needs no new primitive; `movz`/`movn` execute on this unit's boot path. Item 5's prep found that two of its three experiments rested on an unconfirmed assumption. `LOG.md` |
| **Next after this** | **`S0b` at the bench, and it closes `S0`.** `E3` first because it is free and gates two others. Then `DAY-ZERO` item 6 (build container), item 7 (`hazlint`, now with a fourth check whose fixture has a non-zero expected answer), item 8 (`rlxprobe`) |
| **Blocked on** | **`S0b` needs the device on the bench, and it is the only thing holding `S0` open.** Nothing else at the desk |
...
|---|---|---:|---:|:---:|---|
| **S0** | backup restored and verified; the power experiments written up | 1 | — | `~` | `S0a` `LOG.md` 2026-08-23, drill from copy ② not ③ (C-10) · `S0b` **prep written, bench pending** — `notes/power-and-programmer.md` Part 1 |
...
| C-14 | **Is the push button beside the barrel jack wired to `RESET#`, or is it a GPIO?** `RESET#` is pin 49 on the SoC and is multiplexed with `LED_PORT3` and `GPIOB[5]`, so the pin may be an LED driver on this design. Three absences point at `RESET#` — no rootfs binary names a reset button, the vendor kernel has no reset-button driver (with 13 `gpio` strings present as the control), and this unit's loader polls only the UART — **but three absences are not a wire.** It gates `S0b`'s E4 and E5, and `E3` answers it for free: press it with the console attached and see whether stage 1 prints `Booting...`. | S0 (`S0b`) |
