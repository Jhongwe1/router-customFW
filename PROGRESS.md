...
| **Active step** | **`RUNSHEET.md` session B1** — console validation, one power cycle, zero flash bytes, no new code. Written and waiting for the bench. `DAY-ZERO` items 0–4 closed; item 5 (`S0b`) is `⊘`, **deliberately not done**, reason and category in `notes/power-and-programmer.md` §4 |
| **Last session** | 2026-08-23, desk only. Item 4 closed: the loader **scans** six 64 KiB offsets (C-1), the image check is located in this unit's own code and is silent on failure (C-4's structural half), four unbounded RAM-write paths exist and `J BFC00000` is already a watchdog reset, `movz`/`movn` run on the boot path. Then `S0b` was dropped by decision, and the desk answered most of what it existed to answer anyway. Then C-3's residual: **the JEDEC ID is at `0x8040FBD4`** and one console command reads it. `LOG.md` |
| **Next after this** | **Bench session B1.** Then `DAY-ZERO` item 6 (build container), item 7 (`hazlint`, now with a fourth check whose fixture has a non-zero expected answer), item 8 (`rlxprobe`) |
| **Blocked on** | **`S0` cannot close until `C-10` closes** — copy ③ of the backup has never been read back, and that is a ten-minute desk task, not a bench one. Nothing else |
...
|---|---|---:|---:|:---:|---|
| **S0** | backup restored and verified. ~~three power experiments written up~~ — **`S0b` is `⊘`, and the gate's definition shrank with it** | 1 | — | `~` | `S0a` `LOG.md` 2026-08-23, drill from copy ② not ③ (**C-10, and it is the only thing left**) · `S0b` `⊘` blocked-on-instrumentation + a stated risk decision, `notes/power-and-programmer.md` §4 |
