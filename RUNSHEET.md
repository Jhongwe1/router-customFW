...
**What gets typed at the bench, and what each line is expected to return.** 🔄 **2026-09-16 (`R1z-2`), narrowed, and the handover it records already happened:** what this file owns is the **procedure**, and the safety rules that outlive one seating — loader command semantics, the `FLR` bracket, `G8b`’s forbidden sentence. **What gets typed at ONE seating is owned by that seating’s card** under `bench/<date>/`, and § 4063 of this file is where that was ruled (*one card per POWER CYCLE*). 🔴 量 2026-09-16: this file documents `B1`…`B8`; the record runs to `B23`; **15** seatings have no `##` block here of either form, and the count depends on the rule — 18 / 15 / 13 / 11 by four of them. The handover is stated here rather than left to be inferred from an absence, which is what `RUN-1` was.
...
| **C5** | `00008100` -> *(silent)* -> **`00008000`** -> `UID=0x0000001c` -> **`00008100`** | **PASSES, and it is the causal control the whole of `E5` was for.** A bit cleared by hand came back, and `phy_read()`'s `GIMR \|= 1<<8` at `0x80402FB8` is the only thing that puts it there. **Two findings arrived free in the same transcript.** (a) The middle `DW` answered at all, so **the console does not need the timer** -- the call-graph walk that licensed this cell was right, and it is now measured rather than argued. (b) 🔴 **`GISR` moved `88000004` -> `88000104` -> `88000004`.** Bit 8 is `TCIP`, timer interrupt *pending*: with `TCIE` masked the interrupt could not be taken so it latched, and re-enabling it let the ISR run and ack. **The mask, the latch, the delivery and the ack are all visible in five lines**, and none of it was predicted. 🆕 **2026-09-04: this unpredicted half decided a question asked fourteen days later, and it is the best argument in this repository for writing incidental readings down.** `TMR-2` concluded from seating 11 that `TC1IP` does not latch while `GIMR`'s `TC1IE` is clear. Two enables were clear at once and the reading could not separate them — but **this cell measured a `GISR` pending bit latching while its `GIMR` mask was clear**, so on this die a `GIMR` mask stops delivery and not latching, and the remaining cause is `TCIR` bit 30. `notes/timer-driver.md` § 9.2, `docs/interrupt-map.md` § 3.2 |
...
| **P3** 🔄 | *(R0 only)* `ip -brief addr` in WSL, then **`sudo ip link set <if> up`** and **`sudo ip addr replace 10.1.1.2/24 dev <if>`** | an interface that is **not** `eth0`, and after bringing it up `ethtool <if>` reads **`Link detected: yes`** | 🔴 **2026-08-24: bringing it up is part of the check, not a later step.** `ethtool`'s `Link detected` reports the **netdev**, not the wire — on an admin-down interface it reads `no` whatever is plugged in, and `/sys/.../carrier` returns `Invalid argument` rather than the documented `1`. The board's `PSRP` register is the independent second source and it disagreed with `ethtool` for six minutes before the cause was found. **Without this the `§G` transfer fails on the host side and reads as a board fault.** `eth0` is WSL2's NAT'd vNIC (`172.18.x`). **A TFTP reply comes from a different source port than the request went to**, and WinNAT has no conntrack helper for that, so a transfer through the NAT can hang with the board innocent. The USB GbE adapter is in usbipd's persisted list; attach it, or run `loader-tftp.py` from Windows' own Python (3.10.7, pyserial 3.5, both present) 🔴 **2026-09-08 (seating 16): this row is TWO checks and the serial half is the one that gets remembered. The NIC half was skipped and it cost two `looprun` invocations.** 量: `10.1.1.2/24` **does not survive a re-attach** — attach the adapter to WSL and the address is gone, so `enxfc19286184c9` came up `DOWN` with no address and `S6`'s TFTP had no peer. ⚠️ **That fact was measured at seating 12 and lived only in `LOG.md` until today**, which is why it fired again; it is written *here* now because a runsheet row is a thing an operator reads before power and a log entry is not. 🔴 **And `looprun` cannot catch it for you**: `DEFAULT_HOST = "10.1.1.1"` is a constant and nothing in that tool checks host-side reachability before `S6` — every gate it owns points at the **board** (the burn flag, the staged head) and none at the **host**, so a host fault is reported in the board's vocabulary as `STOPPED at S6`. 🟢 **The instrument that settles it is ARP, not ICMP** 🔄 **and 2026-09-21 (seating 36) added the precondition this clause was missing**: the loader answers ARP **only after `IPCONFIG`** (`G3` in this file has said so since 2026-08-24; `looprun` puts `S5`'s rescue, which sends it, before `S5c`, so every `S5c` pass was taken after one). 🔴 On a bare cold boot it answers nothing, 量 `SPEC.md` `NET-95` — and a cell that reads *the loader is dead* on a healthy board is what that costs. After `IPCONFIG`, the loader answers ARP (`ip neigh show dev <if>` → `10.1.1.1 lladdr … REACHABLE`) and does **not** answer ping, so `ping` reporting 100 % loss here is a **pass** and must not be read as a fault. 🔴 **2026-09-09 (seating 19): this row fired AGAIN, and that is a reading about the ROW and not only about the NIC.** The adapter was attached at 08:32 and never given an address; the operator half of P3 was performed and the NIC half was not, for the second seating running. **A runsheet row that has to be remembered at the right moment is not a guard** — 量: the two times it has bitten, what caught it was `looprun`'s `S5c`, not this row. 🟢 The cost was one `looprun` invocation and **no power cycle**, because `S5c` stops before `S6` and names the host; `notes/dev-loop.md` § 17 owns that reading. ⚠️ **The repair is carried forward rather than claimed**: the check that belongs in a tool is *does this host have 10.1.1.x on a link that is up*, and `looprun` already asks it — what is missing is asking it **before** the operator is told to press power, which is where a wasted ESC window costs a seating rather than a retry. `bench/2026-09-08/CORRECTIONS-block13.md` §4.1、`bench/2026-09-09b/CORRECTIONS-block16.md` §3.1 🟢 **2026-09-16 (seating 24): this row's carried-forward repair was performed, for the first time, and it caught the fault.** The repair as written is *asking it before the operator is told to press power*. 量, before power: the adapter was DOWN with no address and `ip -4 route get 10.1.1.1` resolved through `eth0`, WSL's NAT'd vNIC — the fourth time this state has been reached. It was fixed at the desk, and all three of that seating's `looprun` runs passed `S5c`'s `P-a` and `P-b` on the first try. **The row still is not a guard** — what performed it was a human reading this row, which is exactly what the row says is not enough. |
...
| **the port opens** | one 3-second capture with the board off | **0 bytes**, and the tool says so in three causes rather than one. That is the correct answer with no power, and it separates *the adapter is not there* from *the board said nothing* before the power cycle is spent. 🔴 **2026-09-06 (seating 13): one more ordering rule, and it has a control.** The ESC capture must be **open on the port before the operator touches power**, not merely started in the same breath. `SP-A` holds the whole stage-1 banner; `SQ-A` holds none of it, because the operator reached the switch first and the banner went to a closed port. The cold boot is still attested by the capture's shape — `---Escape booting by user` at t=0.004 s, then `Unknown command !` about 1,700 times, and a board already at `<RealTek>` cannot produce the first line — but the banner is lost. For the third cycle the operator was asked to **wait ten seconds after replying**, and `SR-A` has the banner back. **Three cycles, one variable, outcome followed** |
...
| **P4** ✅ | image ceiling | decompressed image < **5,242,880** bytes | the decompressor writes to `0x80000000` and reads from `0x80500000`; over the ceiling it overwrites its own input (`FW-23`). 🔄 **量 2026-08-29 on the four images: 3,472,384 (quiet, quietm) and 3,546,112 (loud, loudm) — 66.2 % and 67.6 %, margins 1,770,496 and 1,696,768.** 🔄 **2026-09-16: those four are the `R3` images and the ceiling figure has moved since.** 讀 `config/rlxfw-initramfs.tsv:50-52`, 量 2026-09-15 on `up1`: decompressed **3,609,088**, margin **1,633,792**, **68.8 %** — reproduced independently by `rtkimage`'s `vmlinux_img` (`notes/dev-loop.md` § 17b). ⚠️ The image that also carries `/bin/ucost` has not been measured against the ceiling. And it now has a second source that shares no code with `mkinitramfs.py`: Realtek's own `cvimg size_chk`, which `rtkload/Makefile:229` runs on every build, prints *Available size* `0x001b0400` and `0x0019e400` — the same two numbers |
...
| **K7** 🔄 | rung 3: 🔄 **`ping -c 4 10.1.1.2` from the board, with `ip neigh flush dev <if>` first and `tcpdump -i <if> -n -e 'icmp or arp'` running on the host throughout.** *(Original: `ping -c 4 10.1.1.1` … `tcpdump -i <if> icmp`.)* | ≥ 1 reply on the board **and** the echo requests in the host capture | 🔴 **D5, and the host capture is what makes it a measurement.** Replies with nothing in the host capture mean something else answered; requests with no replies mean the driver transmits and does not receive. `R0`'s reference is 2/2 at 3.6 ms and it was the *vendor's* driver. 🔴 **2026-09-06 (seating 13): the `-c 4` in this cell does nothing, and neither does any other count.** 量, five requests in three cells: `-c 4`, `-c 20`, `-c 2`, `-c 7`, `-c20` and an explicit `busybox ping -c 3` **all send exactly four packets**, with the command line echoed intact every time. So every ping cell in this file has been getting the default rather than what it asked for. **It invalidates nothing** — four replies is four replies — but a cell that needs a different count cannot have one from this image. `SPEC.md` `NET-26` 🔴 **Three changes, each with a measurement behind it** (§B5-c4): `10.1.1.1` is the address the **loader** had and nobody has once it is gone; the flush is there because *"a stale loader entry broke the ping outright"* — this file's own § The ARP finding, 量; and `arp` in the filter is the one word that separates *ARP never resolved* from *the driver does not transmit*, which `icmp` alone reads as the same failure |
