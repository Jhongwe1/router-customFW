...
|---|---|---|---|---|
| **M0** | `start address: 0x800036xx` | **before the kernel is entered** — `rtkload/hfload.c:114` | it is `kernelStartAddr` read out of **the image's own header** at run time. This unit's staged image holds `0x80003440` (`FW-23`, 讀). ⚠️ It does **not** discriminate against the *drop's* kernel — that kernel is not on this device | **computed** |
...
| **P2** | `hazlint` over every `.o` the build produced under `arch/rlx` | 0 objects with violations. ⚠️ **Still not run, 2026-08-28 (fifth segment)** — P1 covers the same code once linked, but P2 is the check that the assembler's incidental `-march` default (`TC-21`) is still in force, and a linked image cannot tell you that. **It is owed, and it is now owed on FOUR trees rather than one** (`quiet`/`loud` x marked/unmarked) | `TC-21`: **eleven** load-use hazards in hand-written assembly prevented by the assembler's default `-march`, not by the author. **That safety is incidental and this is where it is asserted.** 🆕 And `TC-27` adds a reason it matters more than it did: the `.S` enumeration behind `TC-21` was **17** and is **18**, because `find` does not follow `arch/rlx/bsp`. The eighteenth is a linker script and changes no count — but P2 reads the objects the build actually produced, which is the one check that does not depend on anybody's enumeration being complete |
| **P3** ⚠️ **NOT RUN on any of the four images** | the desk execution channel (`TC-23`), on **the image being uploaded**. 🔴 The only channel run on disk is from 18:02 on 2026-08-28, on the **pre-flag, pre-initramfs, pre-marks** build (3,340,287 bytes). The four images this seating can draw from are 3,968,113 / 3,968,240 / 4,042,261 / 4,042,388 and **none has been through the channel** | reaches `bsp_setup` → `bsp_swcore_init` → `bsp_machine_halt`, ~1,000 KSEG0 instructions, same as this unit's own kernel | a fault before that point is mine and costs no power cycle. ⚠️ **Reaching it is not D1–D5**: qemu's 4Kc has load interlocks and no RTL8196E. 🆕 **And the channel is now worth more than it was**: it stops at `bsp_swcore_init`, which is exactly where B07 is placed, so a channel run would put a value on B07 before the board is plugged in |
