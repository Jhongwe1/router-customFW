...
| `CLK-15` 🆕 | `Booting...` 之後的靜默 | **344.7–356.9 ms**（n=9，平均 349.6，全距 3.5 %）—— loader 印完 `Booting...` 與其後的 NUL 之後靜默，再印 `chipName:`。**冷、暖重置皆同，與重置方式無關。** 🔴 **這才是原本被誤記成「重置 → console 第一個 byte」的那個量** —— 它開始的時候 console 已經在說話了。*推*：DRAM init／校準，成因未定 | 量 | 推 | 量 ×9，跨 `bench/2026-08-24`、`24b`、`24c`、`24d`、`24f` 每一份含開機的擷取 | `RUNSHEET.md` Results — seating 2 part three |
...
|---|---|---|---|
| `CLK-15` | `Booting...` 之後那 350 ms 的靜默是什麼 | 它與重置方式無關（冷、暖皆同，n=9，全距 3.5 %），所以是 loader 自己在做一件固定的事。**讀 `0x80400000`–`0x80400440` 之間的開機路徑**：`Booting...` 的 `dprintf` 到 `chipName:` 的 `dprintf` 之間那段程式，靜態就能定案，不必上機 | R1 |
