...
| `MEM-15` 🆕 | 短時間斷電後 DRAM 保留**內容** | 🔴 **保留，而且有正控制。** `0x80A00000` 在拔電後讀回我上一次拔電用 `FLR` 寫進去的 flash loader 頭部，**16 個字逐位元組相同** —— 寫入者不是推論出來的，是我自己下的指令。對照：**16 小時斷電後零殘留** —— part two 寫下的 23 個特徵字（`C7A0000n`/`C7B0000n`）一個都沒留（`X1`、`X2`）。**所以任何 canary 跨拔電的結論都必須說明斷電多久** | 量 | — | 量 `G0-head-24d` vs `G8pre-rd0` | `PROGRESS.md` `C-17` |
...
| `NET-13` | 哪一個 RJ45 是哪一個埠號 | 🔴 **未定 —— 2026-08-24 重新打開，而它一天之內錯了兩次。** 錯因：五次換孔的孔別是**事後指派**的，而寫入時假設了「依實體位置依序走過」。**量到的是暫存器，不是孔別。** `E11a`–`E11e`（24b）五次讀數各只有一埠 bit 4 起來，集合覆蓋 {0,1,2,3,4}；`E10c`/`E10d`（24c）量到一條線從點亮 `PSRP1` 的孔移到點亮 `PSRP2` 的孔。**兩次的孔別都是事後指派的。** `E10d` 尤其不能當反證：舊圖與線性圖在孔 2 上**預測相同**，而 `PREDICTIONS-block0b.md`（早於該擷取 7 分 08 秒）正是**用舊圖**推出它的期望值 `000010F9` 並命中 —— 一個不會失敗的對照。另有一個更省的解釋未被排除：`E11e`（04:17）把線留在點亮 `PSRP1` 的孔，16 小時斷電期間沒人動它 —— 那樣 `E10c` 反而是舊圖的**確認**。🔴 **兩個方向的導出句都收回**：「照絲印取 index 的驅動會在剛好一個孔上出錯」與它的收回，今天都沒有證據。**站得住的只有兩件**：五個 RJ45 對埠 {0,1,2,3,4} 是雙射（word 6 = `0x000000E2` bit 4 恆為 0，跨 7 次換線不動，而 0–4 每一埠都至少亮過一次）；因此**任何「孔 N = 埠 N」在孔以 1..5 編號時都不可能成立，寫這句話之前必須先講清楚編號基底**。**決定它的實驗**：換孔時把孔別寫進 `--out` 檔名，四次移線四份擷取 | 量 | — | 量 `E10c`/`E10d`/`E11a`–`e` + **D** Table 62 | `docs/loader-phy-and-switch.md` §7 |
...
| `CLK-08` | 看門狗的實際牆鐘時間 | 🔴 **量到了，兩個 `OVSEL` 點，而區間要寫成區間。** 逾時 = **1122.5–1149.9 ms**（`OVSEL=1001`，2²⁴）與 **549.6–577.4 ms**（`OVSEL=1000`，2²³）—— 下界是「命令回音的 read#0 → 最後一個 ESC 回音」，上界再加一個心跳；心跳被工具自己的 `drain(0.02)` 量化在 **20.35 / 20.32 ms**。算出來是 **1174.376 / 587.188 ms**，兩者都短。**分頻／未分頻沒有疑義** —— 未分頻候選 83.9 ms，差 14 倍。🆕 **與時間戳完全無關的獨立確認**：兩格回音的 ESC 個數是 **56 對 28 = 恰好 2.000**，這確認了 `0x240000`／`0x40000` 的 `OVSEL` 分裂欄位解碼，不依賴任何一個時間戳。殘差見 `CLK-08b` | 量 | 讀 | 量 `D4`、`D4c` + **D** Table 27 | `RUNSHEET.md` `D4`；`bench/2026-08-24c/D4.timing`、`D4c.timing` |
| `CLK-08b` 🆕 | 看門狗實際數的時脈 | **14.53–15.26 MHz**，對照計時器自己的 `base/CDBR` = **14.286 MHz** —— 看門狗**至少快 1.017 倍**，而倍率與成因**都未定**。🔴 **「比例而非固定」這個結論被 2026-08-24 的反駁稽核推翻，連同它的數字一起刪掉。** 這次實驗分不出兩個假說：比例假說預測 `D4c` 的短少量是 `D4` 的一半，固定假說預測兩者相等，而兩者只差約 15 ms —— **小於 20 ms 的心跳格點**。單一固定延遲 **L ∈ 24.5–37.6 ms** 對兩點的吻合程度與尺度因子完全相同。而 `RUNSHEET` `D4` 自己設計的估計式（`D4 − D1`，開機時間對消）給出短少量 37.2 / 24.9 ms，比值 **1.495** —— 比起比例假說的 2.00 更靠近固定假說的 1.00。*（原文還帶一個算術錯誤：「固定延遲會給出 0.975 與 0.952」——`L` = 29.4 ms 給的是 0.9750 與 **0.9499**，沒有任何單一 `L` 能同時給出那一對。）* | 量 | — | 量 `D4` + `D4c` | `RUNSHEET.md` `D4` |
| `CLK-16` 🆕 | ESC 心跳的**實得**週期 | 🔴 **要求 20.00 ms 實得 20.35／20.32；要求 2.00 ms 實得 2.32 ms；要求 0.1 µs 實得 29 µs。**差額是主機端一個約 **0.32 ms** 的固定額外開銷，兩個格點各自量到同一個常數。**這是每一份用 ESC 回音量出來的區間所被量化到的格點**，所以它現在由 `console-capture.py` 逐次擷取量測並寫進該次的 `.meta.json`，而不是由讀者假設成請求值。29 µs 是這台主機上這個迴圈的地板 | 量 ×3（pty） | — | 量，`tools/test-console-capture.sh` P11／N15／N16 | `tools/console-capture.py` |
...
| `CPU-35` 🆕 | `Status.IsC` 在這顆上到底有沒有隔離快取 | `probe1` cell 4 的 verdict：`V_CORRUPT` = 那些 byte store 進了真記憶體（qemu 的 24Kf 就是這樣，量於 2026-08-25），其他 verdict = 有隔離。**這台自己的 bootcode 從來沒用過 `IsC`，只有廠商 kernel 用**，所以它跟在 qemu 上一樣未確立 | R1d |
| `NET-13` | 哪一個 RJ45 是哪一個埠號 —— 讀數是量的，孔別從來沒有被記錄過 | 四次移線、四份擷取，**孔別寫進 `--out` 檔名**，讓標籤跟讀數一起躺在檔案系統裡。舊圖 `0,2,3,4,1` 與線性圖只在孔 1 與孔 5 分歧，所以那兩孔各一次就足以定案 | R6 |
| `CLK-08b` | 看門狗數的時脈比 `base/CDBR` 快多少，以及殘差是固定還是比例 | 🔄 **不是第三個 `OVSEL` 點** —— `OVSEL=0111`（算出 293.594 ms）在目前格點下比例預測 286.2、固定預測 263.6，只差 22.6 ms ≈ 一個心跳，分不出來。**要的是更細的心跳**：把 `console-capture.py` `--esc-after` 迴圈的 `drain(0.02)` 改成 `drain(0.002)`，格點 20 ms → 2 ms（38400 下 ESC 加回音來回只有 0.52 ms），現有的兩點就足以分辨。*推，未校準*：代價是約 570 個 ESC 會四次撞上 128 byte 懸崖，每次約 8 ms 無回音，重置落進盲窗的機率約 3 % | R4 |
