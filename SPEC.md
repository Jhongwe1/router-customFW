...
| `CLK-15` 🆕 | `Booting...` **之後**的靜默 | **344.7–356.9 ms**（n=9，平均 349.6，全距 3.5 %），冷暖重置皆同 —— 🔄 **2026-08-25 加兩點成 n=11**：`A-catch` 0.356 s、`bench/2026-08-24c` 0.352 s，都在區間內。🔴 **而同一天發現 `Booting...` 之前也有一個 ≈345 ms 的靜默，只在冷上電時出現**（0.340 / 0.349 s，n=2；暖重置是 0.001 / 0.010 s 且沒有那個 artifact byte）—— **兩個等長且相鄰的區間**，見 `CLK-14`。本列只擁有**之後**那一個。🔴 **2026-08-25 定案，而 §17 為它寫下的實驗指錯了 binary。** `Booting...` **不是 stage 2 印的** —— `Booting` 這個字串在 `stage2.bin` 裡 0 次、在 4 MiB dump 的 flash offset `0x3DE` 出現一次，由 **stage 1** 在 `0xBFC003A4–0xBFC003D0` 用一個沒有 THRE 輪詢的 byte 迴圈直接寫 `0xB8002000` 印出來。所以那 350 ms **跨了一個 stage 邊界**，讀 `0x80400000–0x80400440` 永遠讀不到它。**主導項是 stage 1 在 `0xBFC001D0–0xBFC001EC` 的複製迴圈**：把 flash `0x0004F0–0x0056AB`（20,924 bytes、5,231 圈）一次一個字搬進 `0x80100000`，而迴圈的 8 條指令**與**它的來源字每一個都是從 memory-mapped SPI NOR 來的未快取 KSEG1 讀。**解壓縮是用量測排除的**，不是用論證：同一組 LZMA 參數解 976,877 → 3,374,772 bytes 的 kernel 花 **1.0132 / 1.0233 s**（`bench/2026-08-24c/G6.timing`、`24d/G7.timing`），依輸出位元組與依輸入位元組兩種換算各給 **17.1 / 18.1 ms**，約 5 %。DRAM read-window 訓練掃描（`0xBFC001FC`，≤32 圈）與 BSS 清零各在 2 ms 以下 —— **原本這一列猜的「DRAM init／校準」是錯的**。🔴 **兩個沒藏起來的鬆端**：`Booting...` 之後那個 NUL **不是**那個 byte 迴圈寫的（迴圈在 NUL 上 `beqz` 退出），來源未定；而 3.5 % 的開機間散布對固定時脈的指令執行來說太大，未解釋 | 量 ×9 | 讀 | 量 ×9 + 讀（stage1／stage2／dump）+ 量（解壓縮換算） | `docs/loader-command-semantics.md` |
...
| `CLK-14` 🆕 | 重置 → console 第一個 byte | **2.07 ms**（`D1`，`J BFC00000`：最後一個重置前 byte 在 t=0.015723，第一個重置後 byte 在 t=0.017792，同一份擷取）· **≤21.1 ms**（`D4`）· **≤19.2 ms**（`D4c`）—— 後兩者是**上界**，由 ESC 回音心跳的 20 ms 格點設限。🔄 **冷上電 2026-08-25 量到了，而量它的是一個原本被當成雜訊的位元組。** 擷取先開、電源後上時，CP2102 會交出**一個 framing artifact byte**（今天 `0x00`、`bench/2026-08-24c` `0xFF` —— 同一條線的兩個靜止極性，這也是為什麼兩次不同；裝置印出來的字元兩次會一樣）。那個 byte 是「線被驅動起來」的時刻，而下一個 byte（`\r\nBooting...` 的 `\r`）在 **0.349 / 0.340 s 之後**，n=2。暖重置沒有 artifact byte 也沒有這個間隔（`H1b` 0.001 s、`H3a` 0.010 s）。🔴 **所以 `Booting...` 前後各有一個 ≈345 ms 的靜默，相鄰而等長**，`CLK-15` 擁有後面那一個。**本列原本寫的 340/348/345 ms 於 2026-08-25 被改判給 `CLK-15`；改判是否送對了那兩個相鄰區間中的哪一個，現在是一個桌面問題**（九份擷取都在磁碟上），**未定**。🔴 **原本這一列寫 340 / 348 / 345 ms —— 三個真實存在但被貼錯名字的數字**，見 `CLK-15`；而寫上去的當下就跟本表自己的 `D1b` 矛盾（0.592 s 減掉 `Booting`→橫幅的 0.583 s 只剩約 9 ms，不可能是 348 ms）。**這正是 `D1b` 存在的目的：一個量測戴著另一個量的名字** | 量 | — | 量 ×1 精確 + ×2 上界 | `RUNSHEET.md` Results — seating 2 part three |
...
| `CLK-15` 殘留 | 那個 NUL 是誰寫的，以及 3.5 % 的開機間散布為什麼那麼大 | NUL：讀 stage 1 在 `0xBFC003D4` 之後那一段，以及 16550 的 FIFO 行為。散布：把 `--esc-period` 開到 2 ms 重量一次，看散布是不是儀器的 | R1-gate 順手 |
...
| `CLK-14` 殘留 🆕 | `Booting...` 前後那兩個 ≈345 ms —— 兩個等長且相鄰的靜默，而 `CLK-14` 原本的 340/348/345 ms 在 2026-08-25 被改判給其中一個 | **桌面，九份擷取都在磁碟上**：把 `CLK-15` 引用的那九份逐一分開量「artifact byte → `Booting`」與「`Booting` → `chipName`」兩段，冷開機才有前段、暖重置沒有。**改判送對了哪一個，靠這個決定，不用上機** | R1-gate 順手 |
