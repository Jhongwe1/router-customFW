...
| `REG-23` | `0x8040D4A0` | `AUTOBURN` | **`1`** —— 在提示字元下自動燒錄是**開著的** | 量 | 讀 | S `B6` + A 的初始值 | `RUNSHEET.md` `B6` |
...
| `LDR-06b` | 一行能載多少 | dispatcher 傳的是 `argv+1`，所以一行最多 19 個參數，第一個是位址 —— **`EW` 一行 18 個字 = 72 bytes，寫成 173 個字元**。1 KiB 的裸機探針要 15 行 | 讀 | 讀 | A（同一個斷詞器） | `RUNSHEET.md` `C7` |
| `LDR-06c` | loader 的輸入行緩衝長度 | *(留白)* —— **沒有任何來源量過**，而 173 個字元送進一個更短的緩衝就是往 loader 自己的 `.data` 溢位。要注意的失效模式不是被拒絕，是**被截斷**，或提示字元不再回來 | — | — | `C7` 從 18 個字開始試 | `RUNSHEET.md` `C7` |
...
| `LDR-08` | `EW` | 4 位元組寫入，**不做邊界檢查**，**完全靜默**，而且未對齊的位址會**向上**進位 | 量（靜默，且真的寫在指定位址）· 讀（進位行為） | 讀 | S `C5` + A | `docs/loader-command-semantics.md` §f |
| `LDR-09` | `EB` | 1 位元組寫入，不檢查邊界，靜默，**完全不進位** | 讀 | 讀 | A | 同上 |
...
| `LDR-16` | 中斷手法的副作用 | 排隊的 ESC 會毒到**下一條**命令，讓它回 `Unknown command !`。先送一個裸 `\r` 讀到提示字元再說 | 量 | 讀 | C、S | `upstream/notes/uart-pinout.md` §4 |
...
| `LDR-23b` | 關掉它的語法，以及它到底有沒有生效 | *(留白)* —— **這是 R0 與一次 flash 寫入之間唯一的那道門，而它從沒被證明有效。** 說明字串印的 `AUTOBURN: 0/1` 不是語法，字串表裡 `AUTOBURN` 與 `AUTOBURN: 0/1` 是兩個不同的字串；loader 的回音只是 loader 說它以為的事，真正要讀的是 `0x8040D4A0` 這個燒錄路徑實際查的字。**打錯形式會回 `Unknown command !`，而在一個沒有回讀的流程裡，那長得跟成功一模一樣** | — | — | `C6`：送出後回讀 `DW 8040D4A0 1`，必須是 `00000000` | `RUNSHEET.md` `C6` |
...
| `LDR-23b` | `AUTOBURN 0` 的語法，以及它有沒有真的關掉 | `C6`：送出之後回讀 `0x8040D4A0`，不要相信回音 | R0 |
| `LDR-06c` | loader 輸入行緩衝的長度 | `C7` 從一行 18 個字開始試；要盯的失效模式是截斷，不是拒絕 | R1 |
