...
| **`TM-4a`** | `E5` | `CAP OUT TM-4a --send 'cat /sys/devices/system/clocksource/clocksource0/available_clocksource' --idle 3 --seconds 15` | `TM-3` returned `state=armed` | **`jiffies rtl819x-tc1`** — both, in **rating** order | `rtl819x-tc1` absent → `clocksource_register` returned an error the dump did not show |
| **`TM-4b`** | `E5` | `CAP OUT TM-4b --send 'cat /sys/devices/system/clocksource/clocksource0/current_clocksource' --idle 3 --seconds 15` | as above | **`jiffies`** — still, and the *reason* is the rating, which `TM-4a`'s order shows | 🔴 `rtl819x-tc1` → **rating 0 did not keep it out**, `L2-e`'s enqueue analysis is wrong, **and `E8` is void** because the kernel would then be reading time from the thing `E8` compares against. Disarm; the finding is the reading |
...
| **`TM-7`** | `E9` | `CAP OUT TM-7 --send 'echo disarm > /proc/rtl819x-timer ; cat /proc/rtl819x-timer' --idle 3 --seconds 20` | armed | `state=idle`, `last_verdict=0`, **`tccnr` back to `tccnr_at_init`**, and 🆕 **`tcir_tc1ip=0`** | `tccnr` not returning to its `at_init` value → the driver cannot undo its own write, and that is worth knowing before `R5-3`. **`tcir_tc1ip` still 1 after disarm** → `TC1IP` is not write-1-to-clear on this die, which is `Q11` |
...
⚠️ **`tc1_ext_reads` is a free ordering check and it is predicted, not just read.**
讀: `arm` zeroes the counter and only `rtl819x_tc_read_proc` advances it — the
clocksource's own `.read()` does not, and at rating 0 nothing calls it anyway.
So `TM-3` prints **1**, `TM-5` prints **2**, and `TM-6`'s three dumps print
**3, 4, 5**. A different number means a `/proc` read happened that this card did
not issue, and the card would rather find that out than assume it did not.

⚠️ **`TM-4a`/`TM-4b` do not advance it** — they are `sysfs`, a different file.
...
| **`Q11`** 🆕 | `TM-7` reads `tcir_tc1ip=0` — disarm's `TCIR` write **cleared** the latched bit | it still reading 1 → `TC1IP` is not write-1-to-clear on this part. 🔴 **This is the one write in the driver that rests on a SINGLE source** (D Table 25; the driver's own comment says so), and reading `tcir_tc1ip` afterwards is the only thing that turns that source into a measurement. It costs nothing — the field is already in the dump. ⚠️ Void if `Q10` came out 0: a bit that never latched cannot be shown to clear |
