...
| `2026-09-20` | 0 | `PREDICTIONS-B29-block28.md` | block 28, seating 29, 2026-09-20, `R6-5` — the throughput numbers and the 30-minute flood. Image `r6if1`, `RECIPE_ID` `edc94765`, the first image carrying `iperf3`. The card's own headline is a hazard rather than a result: `notes/nic-driver.md` § 7.6's positive control and § 7.7's refutation condition contradict each other, because `txstall on` holds the queue stopped while `nic_ndo_tx_timeout` deliberately does not refresh `trans_start`, so `n_tx_timeout` increments every 5.00 s with nothing wrong — and § 7.7 makes `n_tx_timeout > 0` the refutation of the design. Resolved by baselining it and reading the delta across the load, rather than by editing the driver |
