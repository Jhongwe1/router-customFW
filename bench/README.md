...
| `2026-09-22` | 71 | `PREDICTIONS-B42-block40.md` | block 40, seating 37, the ninety-ninth segment. **One power press** (the 00:45 cold power-on), one Linux boot of `s99c` (`RECIPE_ID c3cb552b`, `rtl819x-nic 1.2`). 🟢🟢 **`D5` is met**: 17.03 / 17.76 / 17.09 Mbit/s over three runs (spread 4.2 %) against 0.04-0.07 Mbit/s in five `phfollow 0` control runs — `NET-82`'s one-index-for-two-rings bug is real, rate-dependent, and worth **250x**. 🟢🟢 **A driver of mine repaired its own fault**: `recover 1` over an existing wedge emitted `NET-101`'s three marks with nobody typing them, `recov_j_fire - recov_j_arm` = **100** = `recov_jiffies` exactly, and 26 stalls met 26 recoveries with `n_recov_fail` and `n_recov_spurious` both 0. 🔴 **Two of `NET-67 殘留`'s candidates died and neither was replaced** — the run-out mask (`E*`) and the RX pairing (`J*`). ⚠️ The `A*`/`B*` cells are the frozen card; `C*`, `D*`, `E*`, `F*`, `G*`, `H*`, `J*` and `X*` are the seating as it ran, with the departures written into `run-partD.sh` and `run-partF.sh` beside the cells they produced. `*-PING`, `*-LADDER`, `F1`, `H1`-`H3`, `D0-CTRL`, `D1-R*`, `D2-R*` are host-side and carry a `.log` and no `.meta.json` |
