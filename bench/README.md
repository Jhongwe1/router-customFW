...
| `2026-09-20b` | 7 | `PREDICTIONS-B31-postmortem.md`, `PREDICTIONS-B32-block30.md` | seating 30, 2026-09-20, power cycle 1. **Block 31** is the DRAM post-mortem of `NET-59`'s hang, and it **refuted its own validity gate**: three `.text`/`.rodata` windows whose expected content was derived from the ELF *before* the read came back at Hamming distance **47.66 / 52.15 / 53.91 %** from what was written -- complete loss after ~14.5 h unpowered -- so every counter it read was discarded under the rule the card wrote first. What survives is a reading nobody planned: the decayed ground state is **stable** (the same 64-byte window byte-identical 90 s apart) and **address-dependent** (`1/5/7/3`-dominant at `80200000` and `8064BC90`, `A/B/2/3`-dominant at `800F79C0`, `8028ED50` and `803CF3F0`), which extends `MEM-17`'s 35-minute / 2.603 % point to the complete-loss end of the curve. **Block 32** is `R6-5`'s second attempt -- a host-driven burst-depth ladder rather than `iperf3`, because seating 29's own captures refute `SPEC.md` `NET-59` in four places. ⚠️ **Declared: `H1`-`H6` are host-side cells inside the `cells` fence and carry a `.log` with no `.meta.json`**, so `check-predictions` scores them and `capdate` does not count them |
