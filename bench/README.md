...
| `2026-09-16` | 0 | `PREDICTIONS-B23-block22.md` | block 22, seating 24, **frozen before power, zero captures yet** — `R1-pub-3` slot 2 (which is also `CPU-45`'s pre-registration, not a rider on it) plus `R1-pub-4b`'s `D-cost` on one power cycle. Two payloads: `probe3` carrying Group L — a D-cache footprint ladder at six sizes and two arena bases 320 KiB apart, `RB_WORDS` 718 → 754 — and `uc1` carrying `ucost`, `RECIPE_ID` `efa93621`. 🔴 **A directory with a card and no captures is the normal state between a freeze and its seating**, and `capdate`'s `D4` indexes it either way, which is why this row exists now rather than after the bench |
