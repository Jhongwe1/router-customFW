...
| `2026-09-16` | 35 | `PREDICTIONS-B23-block22.md`, `CORRECTIONS-block22.md` | block 22, seating 24, 2026-09-16, **one power cycle against a budget of one**, `check-predictions` **11 of 11** — `R1-pub-3` slot 2 and `R1-pub-4b`'s `D-cost` on one power cycle, three boots of `uc1` and two runs of `probe3`. 🟢 **The D-cache knee is measured**: flat to 8 KiB then 11.72× at 16 KiB, with a second arena base 320 KiB away agreeing to 0 ticks on four of six rungs, so it is a cache and not the scratchpad (`CPU-69`). 🟢 **An uncached read invalidates resident lines** — `l.aba.c1` 545 against 552/46 — which closes `CPU-45` from the timing side while Group C's proxy path read negative a fourth time (`CPU-70`). 🟢 **The emulation surface is priced**: `sync` 988.6 ns and `lwu2` 915.6 ns per iteration on two independent boots, `ll` and `sc` native, so `docs/rlx-isa.md` § 8.2's *exactly one costed row* is refuted (`CPU-73`, `CPU-74`). 🟢 **`4a`'s first repeat is byte-identical** (`FW-74`). 🔴 The card's § 5.5 stop condition fired and the cause is `lad_base()`'s six rungs sharing a base (`CPU-72`); 🔴 the closeout audit produced a false finding by reading a `SPEC.md` row through a truncating view, and it reached the point of the operator agreeing to power on. Off-card: `X1-P3bf2`, `X2-LS`, `X3`/`X5-RB`, `X4`/`X7-NW*`, `X6-*` (the second `probe3` run), `X7-M3` |
| `2026-09-17` | 34 | `PREDICTIONS-B24-block23.md`, `CORRECTIONS-block23.md` | block 23, seating 25, 2026-09-17, **one power cycle against a budget of one**, `check-predictions` **14 of 14** — `P1-3` and `P1-4`, the first run of `mfgtest` on silicon. 🟢 **`P1-3` is 11 of 11 on a good unit** and `P1-4` is **five physical injections, five real kills**, each with its revert measured before the next; the first silicon run of `rtl819x-spi 1.2`'s `rdid` and `h601` verbs (`rdid_id=1C7016 rdid_match=1`, `sig_ok=1 ver=1 hw_sum_ok=1`, no byte of that window printed). 🔴 **Four of the eleven checks could not have gone green on a good unit and two of the five injections could not have gone red on a broken one** — three found before power by reading a third party, the fourth found **by the good unit itself** when `C1-AUTO` returned `8 of 9` with a healthy tick. 🔴 The two causes of that are properties of this shell no fixture can show: a `read_proc` file re-renders under the byte-at-a-time reader (`FW-87`, fingerprint `[ce_live==1]`), and parsing saturates at `INT32_MAX` while arithmetic wraps (`FW-88`), with `jiffies` starting above the saturation point. 🟢 `C10-M28` carries its own refutation: `dj=503 di=503 skew=0` — the OLD criterion satisfied — while the cell took **66.78 s** against 21.9 s healthy. 🟢 `M25` was read in both directions with photons, which the card's one-direction form could not do. 🟢 The carried-forward rate reference is measured: Δ vendor line 13 against Δ mine is **1.000** normal and **9.998** at `cereload 20000` (`FW-91`). 🔴 `FW-89`: `MT-ID`'s verdict line is ungreppable in 8 of 8 captures. 🔴 A control of mine covered less and printed the same green (`C1`, 36/5 → 22/2, caught by `C1b`). **Zero flash writes, `n_writes 0` in all eight `MT-FLASH-2` readings, and `X8-M0`'s 32-group map is byte-identical to `2026-09-09b` and `2026-09-10` (`ae87ac03269985d6`).** Off-card: `C1-AUTO2`, `C4-BTN2`, `X6a-*`, `X6b-lockon`, `X8-M0`, `X10-*`, `X11-final` |
| `2026-09-17b` | 93 | `PREDICTIONS-B25-block24.md`, `CORRECTIONS-block24.md`, `PREDICTIONS-B26-block25.md`, `CORRECTIONS-block25.md`, `PREDICTIONS-B27-block26.md`, `CORRECTIONS-block26.md` | blocks 24–26, seating 26, 2026-09-17, **one power cycle against a budget of one**, `check-predictions` **15 of 15**, **2 of 2**, **7 of 7** — `R6-1`: `CPU-45` answered with a real bus master and the answer is NOT COHERENT; `NET-10` closed on a vendor header that had been in this repository all along; eleven switch registers read in both loader and Linux state for the first time; `FLS-26`'s attribution bracket closed for a five-minute vendor-firmware interval |
| `2026-09-19b` | 148 | *(none — this seating froze no card)* | seating 28, 2026-09-19, **two power events**: the initial power-on, and one cold boot agreed in advance whose own result is `NET-54`. `R6-3` and `R6-4` both closed. 🟢 **The descriptor layout was closed at the loader prompt before any driver ran** — cold read, a 1 MiB TFTP through the loader's own RX path, then the same two windows again; `CPURPDCR0` and `CPURMDCR0` each +8 in lockstep, `CPUTPDCR0` +4, OWN 1→0. 🟢 **The four-rung ladder walked**, rung 0 refuted with its positive control in the same boot. 🟢 **`ping` 4/4 both directions on `rlx0`**, `nd_stats rx 5/452 tx 5/452` against 4×98+60. 🔴🔴 **And a convincing run-out diagnosis was refuted by the single-variable A/B built to prove it** — the unfixed image on the same cold-booted hardware ran 0.0531 % against the fixed image's 0.0536 %. ⚠️ **No card was frozen**: predictions were written into each runner script before it ran, so `check-predictions` cannot score this seating and there is no mtime evidence — an exchange of auditability for iteration speed, recorded rather than skipped |
| `2026-09-19` | 70 | `PREDICTIONS-B28-block27.md`, `CORRECTIONS-block27.md` | block 27, seating 27, 2026-09-19, **one power cycle plus one extra power press my own mistake cost**, `check-predictions` **41 of 41** — `R6-2` closed. The switch driver's first execution: boot capture **1,759 bytes against a prediction of 1,759**, `RLXFW-ID0=F681F8E0`. 🟢 **`D2` holds on two registers** (`SWTCR1`, `VCR0`) with **0 of the 28 non-`dumb` registers moving**, and the sharp part is that `FULL_RST` already leaves seven of the nine at exactly the values `dumb` writes — `R6-2`'s named failure mode as a measurement. 🟢 **Round trip 9 of 9**, so the direct register path reads back and `FW-52` does not generalise. 🟢 **Five PHYs at MDIO 0–4** by four register families sharing no code, and the PHY is identified for the first time. 🟢 The ASIC MIB and the netdev counters agree exactly — `D3`'s two-source shape. 🔴 **`SWINTSET` refuted** with an off-card control proving the write path works, so `R6-3`'s rung zero does not work as designed. 🔴 `PHYR` with no arguments hung the loader and the repository had predicted it in August. Off-card: `X0`–`X24` |
| `2026-09-20` | 78 | `PREDICTIONS-B29-block28.md`, `PREDICTIONS-B30-block29.md` | block 28, seating 29, 2026-09-20, `R6-5` — the throughput numbers and the 30-minute flood. Image `r6if1`, `RECIPE_ID` `edc94765`, the first image carrying `iperf3`. The card's own headline is a hazard rather than a result: `notes/nic-driver.md` § 7.6's positive control and § 7.7's refutation condition contradict each other, because `txstall on` holds the queue stopped while `nic_ndo_tx_timeout` deliberately does not refresh `trans_start`, so `n_tx_timeout` increments every 5.00 s with nothing wrong — and § 7.7 makes `n_tx_timeout > 0` the refutation of the design. Resolved by baselining it and reading the delta across the load, rather than by editing the driver 🔄 **block 29 加進來：**第一張卡得 **8 of 45**（座位轉成診斷），第二張重跑 `D5`/`D6` 並在 `D14` 遇上 `NET-59` 的掋住。43 個 `X*` 是明確宣告的 off-card 診斷格。 |
| `2026-09-20b` | 94 | `PREDICTIONS-B31-postmortem.md`, `PREDICTIONS-B32-block30.md`, `CORRECTIONS-block30.md`, `PREDICTIONS-B33-block31.md`, `CORRECTIONS-block31.md`, `PREDICTIONS-B34-block32.md`, `CORRECTIONS-block32.md` | seating 30, 2026-09-20, **one power cycle**, two boots (the second by `busybox reboot -f`, not a power press), blocks 31 and 30 and 31 and 32. **Block 31** is a DRAM post-mortem whose own validity gate refuted it — Hamming distance **47.66 / 52.15 / 53.91 %** from the ELF-derived expectation after ~14.5 h unpowered, so every counter it read was discarded under the rule the card wrote first (`MEM-18`). **Block 30** refuted all three of its pre-registered hypotheses and found the real one: `NET-61`, a multi-frame burst desynchronises the engine's two RX position registers by exactly 4 and the driver indexes both with one variable, so frames are delivered carrying another frame's length; `ip_rcv` drops them as `InTruncatedPkts`, a counter that is **not in `/proc/net/snmp`** (`NET-62`). **Block 31** ran a ladder with no known-good rung and all six of its rungs are confounded — the card's own defect, recorded in `CORRECTIONS-block31.md` § 0. **Block 32** got the fresh-boot baseline back (20/20, `NET-63`) and then `arm` without resetting `nic_rx_idx` left the board silent (`NET-64`). 🔴🔴 **An adversarial pass then refuted more of this seating's own draft rows than the draft had correct results, and the biggest of them dissolves `NET-59`: the board was never hung, it was draining a queue of 189-second `connect()` calls** — `CORRECTIONS-block32.md` § 2. ⚠️ **Declared: the 18 host-side cells (`H*`, four `L*`, `R8-B0`, three `Z*`) carry a `.log` and no `.meta.json`**, so `check-predictions` scores them and `capdate` does not count them; and the `X*`, `Z*` and `Q*` cells are explicitly off-card |
| `2026-09-21` | 177 | `PREDICTIONS-B35-block33.md`, `CORRECTIONS-block33.md` | block 33, seating 31, `R6-5`'s third seating. Image `s31b`, `RECIPE_ID` `f179cf21`, **four cold power-ons**, `check-predictions` **29 of 29**. 🔴 **The card's attribution was refuted by its own block**: it was built because seating 30 named `NET-61` as what stops `D5`, and `n_dsync` read **0** through all three wedges with the detector's three-way positive control (0 / 4 / 1) proven in the same boot. What stops `D5` is `NET-67` — sustained TCP leaves all four TX descriptors engine-owned, `tx_stopped 1` with `n_tx_wake 0`, while `n_rx` climbs to 38: **the board is mute, not deaf**. 🟢 `NET-68`: the new `arm` reclaims the ring, so the fault no longer costs a cold power press, though the recovery is not durable. ⚠️ **DECLARED: this directory SPANS two dates** — 46 captures dated 2026-09-20 and 13 dated 2026-09-21. `capdate`'s `D2` reports it and its own `C3` asserts a midnight-crossing seating must be `SPAN` rather than `RED`. **Not renamed**, because the frozen card cites these paths; the cause was the operator tracking the clock by estimate instead of re-measuring it, and `looprun` ran at 23:16 rather than the 00:26 believed. ⚠️ The `W*`, `X*`, `F*`, `R*`, `E*`, `G*`, `H*` and `V*` cells are explicitly off-card; the host-side `*-PING` cells carry a `.log` and no `.meta.json` |
| `2026-09-21b` | 87 | `PREDICTIONS-B36-block34.md`, `CORRECTIONS-block34.md`, `RESULTS-block34.md` | block 34, seating 32, `R6-5`'s fourth seating. Images `s31b` (quiet) and `s31L` (**loud, first execution on this silicon**), `RECIPE_ID f179cf21` for both — told apart by the assembled image's sha256 and never by `RECIPE_ID` (`FW-99`). **Three power cycles**, of which the second and third were recoveries rather than experiments. 🟢🟢 **`D6` met**: 31.66 min, `drop 0/0`, zero console bytes on a `PRINTK=y` image, 7.067 GB both ways at 29.761 Mbit/s aggregate. 🔴 **`D5` not met**, and for the first time the reason is a mechanism: a TCP connection is the one factor common to every wedge and absent from every non-wedge, and `iperf3` always opens one. 🟢 First reading of the SWCORE descriptor-diagnostic block (`GDSR0` etc.) in this project — `H-POOL` and `H-FC` refuted against conditions written before power. ⚠️ The `B5-*`, `B6-*`, `D2-*`, `X*`, `G*`, `H*`, `K*` and `F9`/`J*` cells are off-card or attempt-renamed; `CORRECTIONS-block34.md` § 1 lists every departure. The `*-PING`, `*-FLOOD32`, `*-BULK`, `*-SMALL` and `F5-HOSTFLOOD` cells are host-side and carry a `.log` and no `.meta.json`; `G0-COLD`, `J0-COLD`, `J2-COLD` and `K0-COLD` are ESC catch windows terminated by `pkill` and have no `.meta.json` either |
| `2026-09-21c` | 0 | `PREDICTIONS-B37-block35.md` | block 35, seating 33. The whole block is `NET-73`'s single ⚠️ and nothing else: separating *a TCP connection* from *the `iperf3` program*, which five committed files name as the first cell of this seating. Image `s31L` (loud, `CONFIG_PRINTK=y`) so that `NET-75`'s 1.06 s console line is a second wedge detector beside `tx_stopped`. A six-rung ladder ordered by how much TCP each rung carries, with the `iperf3` run LAST as the positive control and § 5 declaring the whole ladder VOID if it does not wedge. 🔴 **One correction made before power**: the pre-registration said *`socat` to any listening port*, and `appletcensus` shows this image has no `nc`, no `telnet`, no `inetd` and no `/dev/ptmx`, so *any listening port* was a guess — the ladder is rebuilt around that measurement. ⚠️ The `T3` rung substitutes a host-side Python loop for `socat` to reach a useful connection rate; the substitution is declared in § 4.3 of the card, before the run. The `*-PING`, `*-SYN*`, `T3-SYNRATE` and `T5-BULK` cells are host-side and carry a `.log` and no `.meta.json`; `S0-catch` is the cold-boot ESC catch window and is off-card |
| `2026-09-21d` | 0 | `PREDICTIONS-B39-block37.md` | block 37, seating 34. 🔴 **The bracket block 35 did not have.** Seating 33 measured the switch registers only AFTER the failure, so *“they differ from probe”* was a fact about the whole boot and *“`iperf3` changed them”* was a claim no capture supported. This block dumps all 37 twice before the event (`C2-SWPRE` before `ndo_open`, `C4-SWPRE2` after it) and once after (`C8-SWPOST`). The `C2`/`C4` pair is also a free control on whether `PSRP` bit 12 is clear-on-read. ⚠️ `R6-6`'s readable half rides along: `VCR0`/`VCR1`/`PVCR0`–`PVCR4` are in the same dump, and the card states before power that this is a reading of the VLAN state and **not** `R6-6`'s DoD, two of whose three clauses were proven unreachable before power in seating 32. Image `s32a` (`RECIPE_ID 84385d91`), unchanged from block 36. The `*-PING` cells are host-side and carry a `.log` and no `.meta.json` |
| `2026-09-22` | 71 | `PREDICTIONS-B42-block40.md` | block 40, seating 37, the ninety-ninth segment. **One power press** (the 00:45 cold power-on), one Linux boot of `s99c` (`RECIPE_ID c3cb552b`, `rtl819x-nic 1.2`). 🟢🟢 **`D5` is met**: **four** `phfollow 1` runs — 17.03 / 17.76 / 17.09 Mbit/s clustering (spread 0.73, **4.2 %**) and a **fourth, `H3`, that collapsed after its first interval** — against 0.04-0.07 Mbit/s in five `phfollow 0` control runs. 🔄 **2026-09-22（第一百段）：這一格原本寫「over three runs」，而 `notes/nic-driver.md:2038` 逐字禁止它** —— *The three-run figure may not be quoted without the fourth* —— 因為 `H3` 是同一條臂、同一個命令、同一塊板子，把它排除在外是事後訂的判準。**n ＝ 4 而其中一次崩掉，才是這個量測** — `NET-82`'s one-index-for-two-rings bug is real, rate-dependent, and worth **250x**. 🟢🟢 **A driver of mine repaired its own fault**: `recover 1` over an existing wedge emitted `NET-101`'s three marks with nobody typing them, `recov_j_fire - recov_j_arm` = **100** = `recov_jiffies` exactly, and 26 stalls met **25** recoveries, `n_recov_fail` **0** and `n_recov_spurious` **1**. 🔄 **2026-09-22（第一百段）：原文寫「26 recoveries … both 0」，而 `bench/2026-09-22/E5-RST.log` 量的是 `n_recov_arm 26`／`fire 25`／`ok 25`／`wake 25`／`fail 0`／`spurious 1`** —— 兩條恆等式無殘差合攢：`fire 25 + spurious 1 = 26 = arm`、`recov_wake 25 + n_tx_wake 1 = 26 = n_tx_stop`。`CORRECTIONS-block40.md` § 5.1 已經記了，而這個索引沒有跟上。 🔴 **Two of `NET-67 殘留`'s candidates died and neither was replaced** — the run-out mask (`E*`) and the RX pairing (`J*`). ⚠️ The `A*`/`B*` cells are the frozen card; `C*`, `D*`, `E*`, `F*`, `G*`, `H*`, `J*` and `X*` are the seating as it ran, with the departures written into `run-partD.sh` and `run-partF.sh` beside the cells they produced. `*-PING`, `*-LADDER`, `F1`, `H1`-`H3`, `D0-CTRL`, `D1-R*`, `D2-R*` are host-side and carry a `.log` and no `.meta.json` |
| `2026-09-22b` | 41 | `PREDICTIONS-B43-block41.md` | block 41, seating 38, the hundredth segment. **One power press** (the 21:38 cold power-on), three Linux boots of `s100L` (`RECIPE_ID 82724c8f`, loud, `rtl819x-nic 1.3`, `rtl819x-switch 1.1`) with the two extra boots bought by `busybox reboot -f`. 🟢🟢 **`D4` rung 1**: `ifconfig eth0 up` returns `SIOCSIFFLAGS: No such device` while `/proc/net/dev` still lists `eth0`…`eth4`, `rlx0` opens in the same boot and `ping` is 4/4 — the vendor's `re865x_open` is four instructions in the artefact against `s99c`'s 0x378 (`NET-106`). 🔴🔴 **The headline is a refuted prediction**: `phfollow` is the compiled default and took (`n_ph_used` **136** against the control arm's **0**), and the throughput died anyway because `recov_mode` is **0** at boot — a second verb dependence shipped while fixing the first (`NET-107`). 🔴🔴 **`NET-67` candidate 8 dies on a single-variable A/B**: three TX bases at zero is not the cause, 1.138 s against 1.124 s to first miss (`NET-108`). 🟢 The engine side was read at last (`NET-109`), and `NET-78`'s state was transient tonight (`NET-110`). ⚠️ `A*`–`C*` are the frozen card, with `C2-OFF` and both `NB blast` lines corrected at the bench and declared; `X*` are off-card. `A5-PING`, `B2-IPERF`, `B5-IPERF0`, `C4-DOSE`, `C8-DOSE`, `C9-PING` and the `X*-TCPDUMP` pair are host-side and carry a `.log` and no `.meta.json` |
| `2026-09-23` | 230 | `PREDICTIONS-B44-block42.md`, `CORRECTIONS-block42.md` | block 42, seating 39 (`P2-3`, seating A: both firmwares, cold and warm, through one script, and the throughput matrix). Twelve presses, 15:09–17:37, every one of the 223 fenced cells captured on the declared date; the three flash-map brackets identical to the prediction; `D2` holds. `CORRECTIONS` records a card defect (`Z9-D2`'s `--tsv`), the off-card server restarts, and a host re-attach whose diagnosis was retracted |
| `2026-09-24` | 0 | `PREDICTIONS-B45-block43.md` | block 43, seating 40 (`P2-4`, seating B: every `P2-3` number measured again, on a clock the host does not slew). Card frozen on this date by the 109th segment, before the seating the owner set for the same day |
| `2026-09-25` | 247 | `PREDICTIONS-B46-block44.md`, `CORRECTIONS-block44.md` | block 44, seating 40 (`P2-4`, seating B): card B (block 43) re-dated after its window passed unrun — the same cells, gates and predictions, with the date, the block and the file names substituted, § 4's Windows-sync rule rewritten and § 0 ⑧ added; `redate_proof` (`$FWRE_WORK/rebuild/s110/redate/`) matched every other line to card B's. Twelve presses, 00:25–02:48, **246 of 246** captured after the prediction; `D2` held; the clock guard refused once and permitted at the declared repeat `Z0-HCG2` (`CORRECTIONS` § 1); the second map bracket was refused on its digest with the map unchanged, and the owner read it before the vendor boots went on (§ 2) |
| `2026-09-25b` | 78 | `PREDICTIONS-B47-block45.md`, `CORRECTIONS-block45.md` | block 45, seating 40's second block, on the owner's decision after block 44: where `rlx0` loses the frames it counts as sent. One press, 03:18–03:35; every experiment bracketed by the driver, the board stack, the switch and the host adapter, with ICMP as the positive control; `eth4` as the other driver; a closing map whose capture ends on the prompt and whose `MB` inserts `awk 1`. `check-predictions` **75 of 78**: the fence misnamed `looprun`'s three single-round artefacts (`CORRECTIONS` § 1). The positive control agreed at every stage; the frame-length experiment lost 1,043 of 1,193 echo replies with 678 `JabberErr` at the CPU port; all 71,828 of the scan's RSTs reached the host adapter; `eth4` lost nothing |
| `2026-09-25c` | 321 | `PREDICTIONS-B48-block46.md` | block 46, seating 41 (`R6b-1`: where on the transmit path a frame of a given length goes wrong, one length at a time). Card frozen on this date by the 111th segment, before its one press |
| `2026-09-26` | 422 | `PREDICTIONS-B49-block47.md` | block 47, seating 42 (`R6b-3`, first card: the fix A/B on one boot, driver 1.5 -- the descriptor read back at the default settings first, E2 at block 45's spacing under the fix, 1.4 and the fix, NET-61's runs at E2's eleven, the fix swept over every length on the wire behind its loopback map, 1.4 at 35 single-length wire sweeps, the mechanism maps in loopback). Card frozen on this date before its one press |
| `2026-09-26b` | 592 | `PREDICTIONS-B50-block48.md` | block 48, seating 42's second block (`R6b-3`, second card, driver 1.5, the image block 47 booted: E2 at block 45's spacing under the fix, 1.4 and the fix for `D2`'s second boot; each of E2's eleven lengths in its own bracket at the fix and at 1.4, 20 requests on a re-armed ring, for `D1`'s stack arm; the fix swept over every length on the wire behind its loopback map, 1.4 at E2's eleven single-length wire sweeps; `NET-111`'s twelve `iperf3` trials at the fix and two at 1.4 for `D3`'s first boot, the board's `Udp:` line around each). Card frozen on this date before its one press |
| `2026-09-21f` | 63 | `PREDICTIONS-B41-block39.md` | block 39, seating 36, the ninety-eighth segment. **One power press** (the 22:22 cold power-on), two Linux boots of `s32a` (`RECIPE_ID 84385d91`, unchanged from blocks 36-38), `check-predictions` **23 of 23**. 🟢🟢 The loader is the reference implementation `R6-5` never had: at its prompt it took **31,475 frames** up to 8 Mbit/s and never stopped answering ARP, where rlxfw goes silent at **347** on the same power-up twenty minutes later. 🟢 **And the frames reached the ENGINE**, which is the half that makes that worth anything: the loader's own receive counter `0x8040EAC4` moved **+5,544** against 5,534 sent, `tx_count` **+6**, `tx_done_idx` **3 → 1 = (3+6) mod 4**, with two reads ten seconds apart and nothing sent between them as the negative control, and every address carrying a code-side second source. 🔴 **The card's own § 1 corrects the framing it was built on**: the loader does not answer ARP at its prompt until `IPCONFIG` has been typed, so no `S5c` reading was ever taken on a bare cold boot (`SPEC.md` `NET-95`). 🔴 The stall is permanent to **600 s** and 🟢🟢 the repair is measured: `engine off` → `arm` → `engine on`, twice from a pristine wedge, back to ping 4/4, with the queue wake done by the ISR level test already in the driver. ⚠️ The `A*` cells are Part A and need no image; the `X*` cells are explicitly declared off-card, and `B0a-SW`/`B0b-UP` are off-card because the card's `B1-PING` targets an interface no card cell brings up — a defect recorded rather than repaired. `tools/netblast.py` is the instrument and lives in `tools/` rather than here because this seating is its second user. The `*-ARP`, `*-PING`, `*-LADDER`, `*-LONG` and `*-BLAST` cells are host-side and carry a `.log` and no `.meta.json`; `S0-catch` is the cold-boot ESC catch window and is outside the `cells` fence by design; the `run-*.sh` files are the runners, committed because they are what was actually typed |
| `2026-09-21e` | 0 | `PREDICTIONS-B40-block38.md` | block 38, seating 35, `R6-5`'s seventh attempt and the first with a **reference implementation**. Image `s32a` (`RECIPE_ID 84385d91`), unchanged from blocks 36 and 37. **Two power presses, eight boots**, `check-predictions` **43 of 52**. 🟢🟢 Boot 1 is `docs/nic-vendor-diff.md` § 10's V ladder and it returned the baseline this gate had never had: the vendor's driver carried `iperf3` — 31.3 MBytes at 25.4 Mbit/s, `Retr 0` — and survived, with the `rlx0` → `eth4` handover measured in the same boot, which refutes `docs/KNOWN-ISSUES.md`'s two `EBUSY` rows on the device. 🔴🔴 Boot 2's `W1` wedged the board while it was the SERVER, and the off-card `Y1` wedged it with UDP at a **closed port** — no TCP, no listener, no userspace — so all three of `NET-77`'s candidates and `NET-73` fall together. `Y5` puts the trigger at **347 frames in 8.01 s at 0.5 Mbit/s** with `n_dsync 0`. 🔴🔴 `Y6` is the single-variable A/B that matters: `tx_mode 1`, the vendor's ring-full contract, polls the OWN bit **1,548 times** with `tx_retry_max_seen 129` and `n_tx_recovered` **0** — the engine's refusal is permanent and no TX-side driver strategy masks it. ⚠️ **Nine carded cells did not run**: `W3`×2 and `W4`×3 were subsumed by `Y1` and not repeated, and the four `W5-*` post-state cells could not run because `W5` killed the shell — which § 2.8 of the card named in advance as a reading. The `L*`, `X*` and `Y*` cells are explicitly off-card; `y5-rateladder.py` is the off-card instrument and is committed beside them; `S0-catch` and `S1-catch` are the two cold-boot ESC catch windows and are outside the `cells` fence by design. The `*-PING`, `*-SOCAT`, `*-IPERF` host cells and `*-TCPDUMP` carry a `.log` and no `.meta.json` |
...
13** rows from the artefacts the card names; `check-predictions` returned **0 of
25**, which is what a block written before its power cycle must say. Block 3's
own §10 records that its 36-of-36 re-derivation was done by an uncommitted
scratchpad script with no controls; this is that script with controls, one
session later.

🔴 **Two read-backs of `K-rdh` and `K-rdc` are NOT in this directory and never
will be.** They are `H601` windows — this unit's MAC and radio calibration —
and `tools/flrbracket.py run` refuses, *before it opens the port*, to write them
anywhere inside this repository. The echoes (`K-flrh`, `K-flrc`) are here,
because an echo holds addresses and no flash bytes.
