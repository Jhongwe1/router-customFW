...
| `2026-09-25c` | 0 | `PREDICTIONS-B48-block46.md` | block 46, seating 41 (`R6b-1`: where on the transmit path a frame of a given length goes wrong, one length at a time). Card frozen on this date by the 111th segment, before its one press |
| `2026-09-26` | 0 | `PREDICTIONS-B49-block47.md` | block 47, seating 42 (`R6b-3`, first card: the fix A/B on one boot, driver 1.5 -- the descriptor read back at the default settings first, E2 at block 45's spacing under the fix, 1.4 and the fix, NET-61's runs at E2's eleven, the fix swept over every length on the wire behind its loopback map, 1.4 at 35 single-length wire sweeps, the mechanism maps in loopback). Card frozen on this date before its one press |
| `2026-09-26b` | 0 | `PREDICTIONS-B50-block48.md` | block 48, seating 42's second block (`R6b-3`, second card, driver 1.5, the image block 47 booted: E2 at block 45's spacing under the fix, 1.4 and the fix for `D2`'s second boot; each of E2's eleven lengths in its own bracket at the fix and at 1.4, 20 requests on a re-armed ring, for `D1`'s stack arm; the fix swept over every length on the wire behind its loopback map, 1.4 at E2's eleven single-length wire sweeps; `NET-111`'s twelve `iperf3` trials at the fix and two at 1.4 for `D3`'s first boot, the board's `Udp:` line around each). Card frozen on this date before its one press |
