...
| `2026-09-25` | 0 | `PREDICTIONS-B46-block44.md` | block 44, seating 40 (`P2-4`, seating B): card B (block 43) re-dated after its window passed unrun — the same cells, gates and predictions, with the date, the block and the file names substituted, § 4's Windows-sync rule rewritten and § 0 ⑧ added; `redate_proof` (`$FWRE_WORK/rebuild/s110/redate/`) matched every other line to card B's. Frozen by the 110th segment after midnight, before the seating the owner set for the same night |
| `2026-09-25b` | 0 | `PREDICTIONS-B47-block45.md` | block 45, seating 40's second block, on the owner's decision after block 44: where `rlx0` loses the frames it counts as sent. One press; every experiment bracketed by the driver, the board stack, the switch and the host adapter, with ICMP as the positive control; `eth4` as the other driver; a closing map whose capture ends on the prompt and whose `MB` inserts `awk 1` (block 44's `CORRECTIONS` § 2). Frozen by the 110th segment |
