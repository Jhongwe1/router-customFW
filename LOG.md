...
### 4. 🟢 `R5-12` ② 有答案了:`rtl819x-wdt` 在 6.18.50 上編得過,4 個 API、3.62 %
...
46 行新增裡 **34 行是 `/proc` shim**(其中 6 行是它的註解),另外三個 API
**各一行**。物件 `elf32-tradbigmips`、22,696 B、178 個符號,五個預期符號全在。
...
🔴 **`gpio` 與 `spi`:普查自己宣告的那個盲點就是大宗。** `gpio` 的 24 個裡有 21 個是
