...
* ⚠️ **兩支現有掃描器都看不到那一行。** `leakscan` 點名了那個檔的二十一行
  (`:215`、`:216`、`:1848`、`:1931`、`:2042`–`:2044`、`:2057`、`:5186`、
  `:5196`–`:5197`、`:5329`–`:5330`,加六行 `enx`),**沒有 2557**;
...
**這不改變 `FLS-22` 的決定**(`upstream/` 不動):同一個檔案本來就在二十一行用冒號
形式印了 MAC,那個決定已經涵蓋。**它改變的是**「沒有任何東西查一份已提交的擷取裡
