...
產物是 `docs/isa-prior-art.md`(43 列 ＋ 三個決定)、`tools/isacensus.py`(導出
...
**四格** —— `CPU-15`、`CPU-17`、`CPU-31`、`TC-h`。**43 列對 4 格。** 差的不是任
一邊的缺陷:§17 數的是 `SPEC.md` 缺*答案*的列,這份檔案數的是 payload 必須攜帶的
列。中間那 39 列是 `SPEC.md` 已經認為答完了的,答在路徑②與③上。

> **也就是說,那四格 `SPEC.md` 留白是這個 gate 的交付物,另外 39 列是它的證據。**
...
### 7a. 🔴 收工前的一次自我追問,把一列的分類改掉了 —— 而它是這一段第二大的結果
...
`check` **43 列雙向**:每一個導出的列都要在表裡(`K1`)、表裡不能有任何儀器導不出
