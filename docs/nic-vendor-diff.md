...
| 11 | TX `m_len` | = `ph_len`: frame + 4, 64 for a runt (`_swNic_send`) | the frame padded to 60, `ph_len` − 4 (`nic_xmit`): 1.4, and 1.5's default; the vendor's value behind `txlen vendor` or `mlen` (`notes/nic-driver.md` § 23) | ⚠️ assessed for `NET-78` on counters, not as a cause (`notes/nic-driver.md` § 26.3–26.4): on blocks 47 and 48 frames counted nowhere appeared only at 1.4's value, each time with a TX stall, and 140 replies at its bad lengths without a stall were all counted; it is `R6b`'s M1 (`notes/nic-driver.md` § 21.6). The loader writes the vendor's value, § 16 |
