...
moved, `led` **0 → 3** *(🔄 **4** on 2026-09-10, the fifty-third segment: `include/linux/leds.h`, in the table below)* and `keys` **1 → 4**, and every path is generic
...
|---|---|---|---|
| `drivers/leds/leds-gpio.c` | line | **generic** | 🆕 `create_gpio_led()` in full, and every `gpio_*` call site: `gpio_request` → `gpio_cansleep` → **`gpio_direction_output(gpio, led_dat->active_low)`** → `led_classdev_register`, plus `gpio_led_set`'s `can_sleep` branch at `:59-63`. It is what says the probe's first write is the LED's OFF level and not its ON level, which is `notes/gpio-driver.md` § 7's central prediction. Mainline's generic GPIO-LED driver: no register address, no bit number, nothing about this SoC. 🔄 **2026-09-10 (fifty-fifth segment): `gpio_led_set` read in full rather than only its `can_sleep` branch** — `value != LED_OFF` gives `level = 1`, then `if (led_dat->active_low) level = !level`, so `brightness 1` drives the pin **low**. That is the one place the polarity is applied on the write path, and `bench/2026-09-10/PREDICTIONS-B18-block17.md` § 5 rests on it. Depth again, not path |
...
| `drivers/leds/Kconfig` | 🔄 **line, widened 2026-09-10** | **generic** | 🆕 `:12-16` `LEDS_CLASS`, `:121-129` `LEDS_GPIO depends on LEDS_CLASS && GENERIC_GPIO`. 🔄 **The fifty-third segment parsed the WHOLE file** — all 35 `config`/`menuconfig` entries with their type, prompt and every `depends on`, plus the `if NEW_LEDS` nesting — to enumerate what becomes reachable when the three symbols go on (`notes/gpio-driver.md` § 9.2). Nothing in it is a register address or a bit number, so the verdict does not move; the RANGE does, and § 2.1's rule is that ranges are what this file records. 🔴 **No checker saw the widening** — `LEDGER-4` is exactly that blind spot |
| `include/linux/leds.h` | line | **generic** | 🆕 2026-09-10 (fifty-third segment). `struct gpio_led` and `struct gpio_led_platform_data` in full — the five and three fields `arch/rlx/kernel/rlxfw-devices.c` fills in. Read to know what a platform device for `leds-gpio` has to carry, which `leds-gpio.c` alone does not say. Mainline's generic LED header: no register address, no bit number, nothing about this SoC. 🔴 **`ledgerscan check` was GREEN over this throughout.** ~~its `TEXT_EXT` is `.md .tsv .json .py .sh .yml .yaml .txt`, and the only citation of this path in the tree is an `#include <linux/leds.h>` in a `.c` file of mine. **A THIRD shape for `LEDGER-4`** — the first was a depth change no scanner can see, the second a file read and never written about, this one a citation in a file type the scanner does not open.~~ 🔄 **2026-09-10 (R5-8): THAT MECHANISM IS REFUTED AND IT WAS FALSE ON THE DAY IT WAS WRITTEN.** 量, two ways. ① `tools/ledgerscan.py:240` has read `TEXT_EXT = (".md", ".tsv", ".json", ".py", ".sh", ".yml", ".yaml", ".txt", ".c", ".h", ".toml", ".cfg", ".conf")` since `aa89317`, **2026-09-03 00:49** — seven days before the sentence above was committed in `02ded87` at 2026-09-10 09:13. ② The scanner **demonstrably reads `.c` files**: on 2026-09-10 it went RED on `drivers/input/evbug.c` and `drivers/char/keyboard.c`, and the citation it found for both is inside `config/rlxfw-src/…/arch/rlx/kernel/rlxfw-devices.c`. **THE REAL MECHANISM IS THE CITATION'S FORM.** 量: the string `include/linux/leds.h` — the path this ledger declares — occurs in `LOG.md`, `PROGRESS.md` and this file, all `.md`, and in no `.c` at all; what the `.c` carries is `#include <linux/leds.h>`, which does not contain it, because the kernel's include path elides the `include/` prefix. ⚠️ **AND MAPPING ONE TO THE OTHER WOULD BE WRONG**, which is why it is not being done: an `#include` is not evidence that a file was READ, and this ledger's subject is what was taken. **So this is not a third shape.** It collapses into the second — a file read on one day and declared on another, over which `check` is green because `check` compares citations and not reads. ⚠️ It does NOT move the in-scope 58 below, which is the SCAN's number and not this table's row count |
...
| `include/linux/gpio_keys.h` | line | **generic** | 🆕 2026-09-10 (`R5-8`). **The whole file, twenty lines** — `struct gpio_keys_button` (`code`, `gpio`, `active_low`, `desc`, `type`, `wakeup`, `debounce_interval`) and `struct gpio_keys_platform_data` (`buttons`, `nbuttons`, `rep`). `rtl819x-keys` CONSUMES this struct unmodified rather than inventing one, so the board's description of its own button is in upstream's vocabulary and `gpio_keys` could accept it — which is what makes *upstream cannot drive this board* a statement about the SoC. Mainline generic: no register, no bit, nothing about this part |
| `drivers/input/evdev.c` | line | **generic** | 🆕 2026-09-10 (`R5-8`). Two places. `:190` — `input_open_device()` sits inside `evdev_open()` and is gated on `!evdev->open++`, so evdev opens the device on the FIRST USERSPACE OPEN of /dev/input/eventN and not at connect; that is the whole reason `CONFIG_INPUT_EVDEV=y` and the `/dev/input/event0` node are in this image, because `input-polldev` queues no poll until something opens. `:11` — `#define EVDEV_MINOR_BASE 64`, one of the two sources for the node's minor |
| `drivers/input/evbug.c` | line | **generic** | 🆕 2026-09-10 (`R5-8`). `:63` and the six lines above it — `input_open_device()` is inside `evbug_connect()`, so evbug opens EVERY device it connects to, at registration. Read to decide against it: it would start the polling with no userspace involved and destroy the `n_open`/`n_poll` three-state reading. `CONFIG_INPUT_EVBUG` is pinned `n` in config/rlxfw-kernel.delta with that reason |
| `drivers/char/keyboard.c` | name | **generic** | 🆕 2026-09-10 (`R5-8`). ⚠️ **NOT OPENED.** What was taken is one number from `grep -c input_open_device` — it is 1 — which puts the VT keyboard handler in the same class as evbug (opens at connect) for the enumeration in `arch/rlx/kernel/rlxfw-devices.c`'s fourth `#error`. Its `CONFIG_VT` is not set in this image, so the branch was never followed further. Depth `name` and not `line` deliberately: a count from a grep is not a reading of the file |
...
| `arch/rlx/kernel/Makefile` | line | 🔴 **vendor** | 🆕 2026-09-10, read in full to decide where `R5-7`'s platform device is staged (`notes/gpio-driver.md` § 9.1): the three `obj-y +=` lines whose form the `rlxfw-marks.tsv` insert has to match, and **`EXTRA_CFLAGS += -Werror`**, which is a build constraint on the first file of mine to live there. ⚠️ Vendor, and declared as such — but an `obj-y` list carries no register address, no bit number and no sequence, so it lands on **neither** layer § 5 scores. It was already cited at `notes/dev-loop.md:226` before today |
| `arch/rlx/Makefile` | line | 🔴 **vendor** | 🆕 2026-09-10, five lines: `:117` `core-y += arch/rlx/kernel/ arch/rlx/mm/` — unconditional, which is why § 9.1 needs no Kconfig plumbing — plus the `drivers-$(CONFIG_PCI)`, `libs-y` and `drivers-$(CONFIG_OPROFILE)` lines around it, read to be sure `:117` was not conditional. Same standing as the row above: vendor, and on neither layer |
| `fs/proc/uptime.c` | line | **generic** | 🆕 2026-09-14 (sixty-eighth segment, block 20's pre-power audit). `:23` — `seq_printf(m, "%lu.%02lu %lu.%02lu\n", …)` and the four arguments under it — read to derive the **wire length** of a `/proc/uptime` read, which block 20's § 5.2 needs and had estimated at ~15 bytes. The format gives `d1 + d2 + 8`, so **12** at a two-digit uptime and idle. 🔴 The first draft of that correction said **13**; it was relayed rather than derived, and deriving it from this line is what produced the 12. Byte-identical in `src-vendor/rtl819x-toolchain/linux-2.6.30/` and in the shipped image's tree, so nothing vendor-specific was taken. ⚠️ **This row exists because `ledgerscan check` went RED on it** — the citation was invented by this segment's own write-up, which is `CLAUDE.md`'s rule 3b (*a sweep can never cover the segment's own write-up*) landing on its own terms; the cheap fix of deleting the path from the prose is the one § below rejects by name |
| `drivers/leds/rtl819x-leds-board.c` | name | **none — a path that names no file** | 🆕 2026-09-10. **This file does not exist in any tree this project has.** It is option C of `notes/gpio-driver.md` § 9.1's three-way table, named so the option could be *rejected* — the path would lie, because the file registers a platform device and is not an LED driver. 🔴 **A fourth shape for `origin: none`, and the purest one**: the first was an *example* and could in principle have been read; this one **cannot** have been, and that is mechanically checkable — the file is absent from `src-vendor/`, from `$FWRE_WORK/modern/`, and from `config/rlxfw-src/`. ⚠️ It is also the shape most open to abuse, because *"I invented this path"* is unfalsifiable in the direction that matters; what keeps it honest is that a rejected design option has to be visible in the prose it was rejected in, and § 9.1's table is that prose |
...
| `drivers/gpio/gpiolib.c` | **line** | **generic** | which ops the framework dispatches to, which is what makes `.request` the right place for the policy: `:804` `gpio_request()` calls `chip->request`, `:994` `gpio_direction_output()` calls `chip->direction_output`, and 🔴 `:967` fails early unless **both** `.set` and `.direction_output` exist — so omitting `.set` to make writing impossible is not available, and `:1065` `__gpio_set_value()` dispatches with no NULL check, which would be an oops rather than a refusal. Also `:1155`'s `gpio_to_irq()` call and the `#ifdef CONFIG_DEBUG_FS` at `:1133` that compiles it out. 🔴 **2026-09-06, seating 15: this row was already open at `line` depth on the very function that refuted the card, and the relevant call was not taken.** `:994` is recorded above as *calls `chip->direction_output`*, and it also calls `gpio_ensure_requested()` — 2.6.30's compatibility path, which sets `FLAG_REQUESTED` and calls `chip->request` for a caller that never asked. So `tryout 5` left line 5 held and `B1-C`'s `claim` was refused by the framework with `EBUSY` before this driver's `.request` was reached, against a card predicting `n_req_ok = 2`. ⚠️ **Confirmed at `artefact` depth rather than by re-reading the source, deliberately** — `gpio_direction_output` at `0x800d637c` calls `gpio_ensure_requested` (`0x800d5b00`) at `0x800d642c` in this image's own `vmlinux` — and the errno was **measured on the board**, not read: routing the write through `cat` made `strerror` print `Device or resource busy`. **A file being open at `line` depth is not the same as its behaviour being known**, which is the general form of this and is why the depth column is not a coverage claim. 🔄 **2026-09-10 (fifty-fifth segment): the sentence above describes ONE branch of `gpio_ensure_requested()` as though it were the whole function, and the other branch is what `R5-7`/`R5-8`'s seating rests on.** 讀 `:77-96` and `:955-1000` in full, which this row had not: the condition is `WARN(test_and_set_bit(FLAG_REQUESTED, &desc->flags) == 0, ...)` and `test_and_set_bit` returns the **old** bit — so the compatibility path runs only on a line **nobody holds**, and on a line a consumer already holds the test is `WARN(1 == 0)`, the block is skipped and it returns **0**, leaving `gpio_direction_output()`'s `if (status)` false and dispatching straight to `chip->direction_output`. 🔴 **The path was already declared and the function was already named, so no scanner could see this** — it is a change of DEPTH inside a declared path, which is `PROGRESS.md`'s `LEDGER-4` exactly, found by a closeout audit and not by `ledgerscan check`, which was green. **The `gpio` domain's path count does not move** |
| `include/linux/gpio.h` | line | **generic** | 🆕 2026-09-10 (`R5-8`). `:1-120`, the whole file. The `#ifdef CONFIG_GENERIC_GPIO` fork at `:6-9` and the ELEVEN `static inline` stubs in its `#else`, including `gpio_to_irq()` at `:98` returning `-EINVAL` after `WARN_ON(1)` and `irq_to_gpio()` at `:105`. Read to settle whether `arch/rlx` has a definition it could reach: it does not — gpiolib `select`s `GENERIC_GPIO`, so on any image with a `gpio_chip` the `#if` branch is taken and `<asm/gpio.h>` is what is read. `:105` is one of the two upstream sources for `config/host-compat/0006`'s `irq_to_gpio` constant |
| `arch/x86/include/asm/gpio.h` | line | **generic** | 🆕 2026-09-10 (`R5-8`). `:30-60`, the tail of its `CONFIG_GPIOLIB` branch: `gpio_set_value`/`gpio_cansleep` as inline wrappers, then `gpio_to_irq()` at `:44` returning `-ENOSYS` under the comment *"Not implemented, yet."* and `irq_to_gpio()` at `:49` returning `-EINVAL`. Read as the precedent for `config/host-compat/0006` and REJECTED for `gpio_to_irq` (a constant is a number this project chose; `__gpio_to_irq`'s `-ENXIO` is produced by `.to_irq == NULL`), ACCEPTED for `irq_to_gpio`, where no alias exists |
| `arch/mips/include/asm/mach-generic/gpio.h` | line | **generic** | 🆕 2026-09-10 (`R5-8`). Read in full and **compared byte for byte** with `arch/rlx`'s copy: md5 `8c6bc68ef836ae3f7b36f01b94d37792` on both. That is what says the missing `gpio_to_irq()` definition is mainline MIPS's `mach-generic` shape copied whole rather than an `arch/rlx` mistake. ⚠️ `config/host-compat/0006` deliberately does NOT patch this copy: the build is `ARCH=rlx` and no file under `arch/mips/` is compiled |
| `arch/mips/include/asm/pmc-sierra/msp71xx/gpio.h` | 🔄 **line** | **generic** | 🆕 2026-09-10 (`R5-8`). ⚠️ **NEVER OPENED IN AN EDITOR, and the depth is `line` anyway because `ledgerscan` was right and the first draft of this row was not.** It said `name`; the scan reported `ledger=name scan=line`, because the prose cites `:36` — and a `grep -n` prints the line's CONTENT, so one line was taken. One line of a tree-wide grep — `:36` defines `gpio_to_irq` — and it is in this ledger only because it is one of the five definitions that make *"the whole tree has no definition"* the wrong sentence. Nothing was taken from it |
...
| `led` | 🔄 **5** *(4 earlier the same day; 0 until 2026-09-10)* | `R5-7` | 🟢 **blind of any implementation ON THIS SoC**, by the vendor or anyone. All four are mainline's own generic LED plumbing — `leds-gpio.c`, `led-class.c`, `Kconfig`, `include/linux/leds.h` — with no register address and no bit number, so they land on neither layer § 5 scores. **The fourth is `origin: none` and names a file that does not exist** — option C of § 9.1's rejected-alternatives table. 🔴 **The verdict on the line above used to read "blind of any implementation" full stop, and that is what stopped being true**: `leds-gpio.c` IS an implementation, of a GPIO-connected LED in general. ⚠️ And the domain's real exposure is not in this count: `notes/gpio-driver.md` § 5 records **seven** unread vendor symbols that materialise the pin `R5-7` drives, left unread on purpose |
