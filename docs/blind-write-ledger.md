...
| `include/linux/leds.h` | line | **generic** | 🆕 2026-09-10 (fifty-third segment). `struct gpio_led` and `struct gpio_led_platform_data` in full — the five and three fields `arch/rlx/kernel/rlxfw-devices.c` fills in. Read to know what a platform device for `leds-gpio` has to carry, which `leds-gpio.c` alone does not say. Mainline's generic LED header: no register address, no bit number, nothing about this SoC. 🔴 **`ledgerscan check` was GREEN over this throughout and structurally could not be otherwise**: its `TEXT_EXT` is `.md .tsv .json .py .sh .yml .yaml .txt`, and the only citation of this path in the tree is an `#include <linux/leds.h>` in a `.c` file of mine. **A THIRD shape for `LEDGER-4`** — the first was a depth change no scanner can see, the second a file read and never written about, this one a citation in a file type the scanner does not open. ⚠️ It does NOT move the in-scope 58 below, which is the SCAN's number and not this table's row count |
