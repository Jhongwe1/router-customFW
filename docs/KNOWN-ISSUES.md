...
|---|---|
| 🔴🔴 **`NET-78` is not identified.** Four candidates now exist with sources; none has been measured | § 8 of that file names the reading for each. ① needs the switch's per-port MIB counters across the fault; ③ needs a build; ④ rides an existing boot. 🔄 **2026-09-26 (`R6b-2`, desk, `notes/nic-driver.md` § 26.3–26.4): `R6b`'s `M1`, the TX length fields, is the lead, on counters only.** On blocks 47 and 48 descriptors taken and counted nowhere appeared only under 1.4's fields, and each time with a TX stall; the vendor's lengths never stalled, so length and stall are not separated. On block 46 no such run is placed after a clean history on bracket-level data — two of the six that could refute follow a bad length, four are undetermined — and "follows a recent fire" fits nearly as well. What would settle it: a stall at the vendor's lengths on one boot, beside 1.4's, with the uncounted frames read on both. `SPEC.md` `NET-78 殘留` |
...
| ⚠️ **Nothing here touched `A3`/`A4`** — the TX pkthdr words 2/3/4 are still absent from the `/proc` dump, and `-DRTL_DEBUG_NIC_SKB_BUFFER` is still not in the board template (🔄 2026-09-26: that flag gates `nic_mbuf` alone; the rings and `pvid` need `CONFIG_RTL_PROC_DEBUG`, `notes/switch-driver.md` § 8.14). Both change `RECIPE_ID` | One image |
...
| 🔴 **The engine-side reading was taken and it does not close.** | `Rcv 0 bytes` on the CPU port's ingress beside `CRCAlignErr` growing +7 for +7 transmitted frames, while port 3's egress also grows +7. The obvious reading — frames leaving with a bad FCS, dropped by the host NIC in hardware — is **refuted** by the host's own `RX errors: crc`, which is 0 before and 0 after. 🔄 **2026-09-26 (`R6b-2`, desk, `notes/nic-driver.md` § 26.1–26.2): read on three TX paths, and it still does not close.** The pattern is not rlxfw 1.4's descriptors: under the vendor's `eth4` driver (block 45) and driver 1.5 at the vendor's lengths (block 47) the CPU port also reads `Rcv 0` and no good-frame count while `CRCAlignErr` equals port 3's egress (16,215 = 16,215; 3,352 over six brackets). Block 46's counters do not refute the desk reading that the CPU port counts every frame from the engine as an FCS error (推), but its zeros have no positive control — no committed capture has one of those counters non-zero — and why is 未定. Port 3's `ByPassTCRC` reads 0, *recalculate the CRC of a CRC-error frame* (two sources), so the host's crc 0 still rules out frames dropped at the host for a bad FCS and says nothing about the FCS the CPU port received (推: which port the bit acts on is not settled). `SPEC.md` `NET-109 殘留` |
