...
| | |
|---|---|
| 🔴🔴 **Why the engine stops retiring a TX descriptor.** Still untouched, and it is still the only question on this line. This seating removed the engine itself as the difference (`NET-98`: the loader survives 16× the dose on the same power-up) and produced a repair that works (`NET-101`), but a repair is not a mechanism | The run-out mask of § 12.4 is the strongest remaining candidate and 讀 `rtl819x-nic.c:966-968` says it **cannot be tested without an image** — the NAPI-complete path ORs those bits back in on every completion. `NET-67 殘留` |
