...
| ⚠️ **`D5`'s three runs come from one evening, one board, one direction.** Host→board only; the board was the server in every run that produced a number. The reverse direction was measured only in the `phfollow 0` arms, where it produced nothing | Three runs in each direction on a later seating, with `phfollow` compiled in |
