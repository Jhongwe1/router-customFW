...
| 🔴 **`PSRP6_RW` differs from `PSRP6` by exactly bit 2, and no source names that bit.** `0000007E` against `0000007A` | `SPEC.md` `NET-41 殘留` owns it. The decisive experiment is a **write**, needs an explicit yes, and `LDR-44` says `EW` is in the loader's unchecked-argument family |
