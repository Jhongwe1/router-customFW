...
predicted"* (讀, `probe3.c:134 (#define RB_POISON_W (RB_WORDS + 8u))`). **`DW 80A02000 641` prints `4 × ceil(641/4)` =
...
| **`t-hit`** | 🔴 **a 4 KiB working set — half the predicted D-cache — traversed 32 times at 16 B stride**, once through KSEG0 and once through KSEG1, each bracketed, with **one warming pass discarded**. **The iteration count carries N, the footprint does not**: after the warming pass every cached access is a hit, so the KSEG0 leg measures **residency** rather than miss latency. ⚠️ **A single sweep of a working set larger than the cache measures miss latency in both windows and is not a residency instrument** — 4096 loads at 32 B stride would touch 128 KiB once, i.e. 4,096 compulsory misses, and *equal* would then be the expected reading on correct silicon | 🔴 **`0` for the warming pass and both legs** (量(qemu)) | 留白. 推: the uncached loop is several times the cached one. N=4096 gives 1,760–5,850 ticks of signal against a 1-tick LSB | equal → either the timer is not counting (contradicts `t-cal`) or KSEG0 is not cached, which would refute M1 by a route with no cache cell in it. 🔴🔴 **2026-09-14: this column names ONE condition and the reading went the other way, so the outcome that arrived had no falsifier written for it.** 量 `t.hit.warm/ks0/ks1` = **552 / 1,472 / 13,698** ticks on three power cycles (0.075 % spread), i.e. **138.43 / 0.07 / 104.54 ns** per load after the loop's own ALU cost — read and interpreted in `docs/rlx-cache-and-cp0.md` § ⓑ-2, which collapses `c-A`'s disjunction to *the alias is snooped*. **By this project's own rule that is an interpretation of an observation and not a result**, and the cell that pre-registers it is a footprint ladder at 1/2/4/8/16/32 KiB whose knee is the D-cache size. 🔴 **Two errors in this row's own left-hand cell, found the same day**: it says *32 times* and *N=4096* while `probe3.c:1534 (A_VSIZE, 32u)` × `cells.S:447 (256 loads x 16 B = 4 KiB)` is **8,192** loads, so the 1,760–5,850 tick band doubles to **3,520–11,700** and the measured 13,698 is **above it**; and § 10's stage-0 row says the arena is initialised, which it is not — `rlx_w_arm` runs in stages 3 and 7 and Group T is stage 2, so `t-hit` loads uninitialised DRAM |
| **`l-ladder`** 🆕 | 🔴 **the SAME walk as `t-hit` at six footprints — 1, 2, 4, 8, 16, 32 KiB — with the load count HELD AT 8,192**, at **two arena bases** 320 KiB apart, each rung a discarded-then-recorded **cold** leg (one pass, every line a compulsory miss) followed by the **warm** leg. `rlx_tc_ladder` is `rlx_tc_walk` with the literal 256 lifted into a register: 量 objdump, the five-instruction inner body is **byte-identical** and both outer spans are 10 instructions, so the ALU correction and the hit floor transfer unchanged | 🔴 **the group self-voids and prints `Group L VOID`** — Group T already found no timer at `0xB8003108` on Malta, so twelve rungs of `FFFFFFFF` would be twelve readings of the host. 量(qemu) 2026-09-16: the VOID line, `cells.void` 9 → **11** | **warm: 1,472 ticks at every footprint that fits, 17,664 at every one that does not — a flat-then-step of 12.00×, and the knee is the D-cache size.** **cold: 138 / 276 / 552 / 1,104 / 2,208 / 4,416, doubling at every rung.** 算 from `t.hit.warm/ks0` (3 power cycles, 0.075 %) | 🔴 **the 4 KiB rung at base 1 is `t-hit`'s KS0 leg to the instruction** — same base, same footprint, same 32 passes — so `l.a.w4k` ≠ **1,472 ± 1** means the LEAF is wrong and no knee may be read off the other ten rungs; likewise `l.a.c4k` ≠ **552 ± 2**. Flat across all six → no cache. Rising at the first rung → smaller than 1 KiB. 🔴 **A knee at 8 KiB alone does not decide it**: D-MEM is 8 KiB too (`CPU-46`), and what separates them is the second base, not the knee. **A cold column that is not linear voids the warm column**, because the walk is then not walking what this row says 🔄 **2026-09-16 (seating 24), RAN, twice.** warm base A **1,496 / 1,480 / 1,472 / 1,485 / 17,405 / 17,404**, base B **1,496 / 1,480 / 1,472 / 1,485 / 17,406 / 17,408** — **flat through 8 KiB then a step of 11.72×**, so the knee is between 8 and 16 KiB and `docs/rlx-isa.md` § 8.1 is confirmed. `l.a.w4k` = **1,472 to the tick**. 🔴 **But this row's cold column is WRONG and so is `lad_base()`'s own comment**: the six rungs share a base with no invalidation between them, so rung *k*'s cold leg enters with rung *k−1*'s footprint resident and is not all-compulsory. Measured cold: **143 / 149 / 296 / 594 / 1,191 / 4,351** at base A, four consecutive rungs at a constant **0.536–0.540** of this row's model. `l.a.c4k` = 296 against 552 ± 2 **fires the leaf check**, and the leaf is nonetheless verified on both paths — the warm one by `l.a.w4k` and the miss one by `l.b.c1k` = **138**, which this row does not name and which is the only genuinely compulsory-miss rung. **The repair is to move the cold half of the leaf check to rung 0.** `SPEC.md` `CPU-69`, `CPU-72`; `docs/rlx-cache-and-cp0.md` § ⓑ-3 |
| **`l-base2`** 🆕 | the same six rungs at `A_LAD2` = `0x80A90000`, **0x50000 = 320 KiB** above `A_LAD1`. A scratchpad is an address window and a cache is not, so this is the discriminator `CPU-46`'s confound needs. 量 compile-time: the separation sets bits 16 and 18 only, and an 8 KiB cache indexes with bit 12 at most, so **every cache index bit is equal at the two bases by construction** | VOID with the ladder | **identical to base 1 at every footprint** ⇒ the structure follows the data, not the address ⇒ a **cache** | a hit floor at **one** base and not the other ⇒ a **scratchpad**, the knee is the D-MEM size and `l-ladder`'s headline is void. Neither base showing a floor ⇒ the run says nothing and refutes neither — and that is decidable because the floor is a **published constant** (1,472, three power cycles) and not a value chosen from this run. ⚠️ 320 KiB is 40× the scratchpad's size **as read, not as measured**; Group M's `m.dmembase`/`m.dmemtop` on the same boot is the independent second answer 🔄 **2026-09-16 (seating 24): the discriminator ran and it says CACHE.** Warm, base A minus base B, per rung: **0 / 0 / 0 / 0 / −1 / −4** — four rungs identical to the tick and the largest difference 0.27 %. A scratchpad is an address window and would show a floor at one base only. 🟢 Group M on the same boot is the independent second answer this row asked for: `m.dmembase 20000000` with `m.dmemtop 00000000`, a base with no top above it. **`CPU-46`'s confound is closed by the second base and not by the knee**, which is what this row said it would have to be. `SPEC.md` `CPU-69` |
| **`l-aba`** 🆕 | KSEG0 warm → KSEG1 over the same addresses → KSEG0 again, **five recorded legs**, with the two `KSEG0 again` readings taken at **1 pass** and at 32 | VOID with the ladder | `l.aba.0` **1,472** (= `t.hit.ks0`), `l.aba.w1` **46**, `l.aba.1` **13,698** (= `t.hit.ks1`), `l.aba.2` **1,472** either way; the measurement is `l.aba.c1` — **552** if the uncached read invalidated the lines, **46** if it did not | 🔴🔴 **the 32-pass form cannot decide this and the specification said it could.** 算 2026-09-16: over 32 passes only the FIRST is cold, so an invalidating read gives 256 misses + 7,936 hits = **1,978** against 1,472 — **1.34×**, where the owner file's own *"~138 ns/load"* implies 17,664. A reader comparing 1,978 to that would have called it warm and concluded NO invalidation, **the opposite of what happened**. One pass separates them **12.00×**. If `l.aba.0` ≠ `t.hit.ks0` or `l.aba.1` ≠ `t.hit.ks1` the cell is void before `l.aba.c1` is read 🔄 **2026-09-16 (seating 24): ANSWERED, and the answer is that the uncached read invalidates.** `l.aba.0` **1,472**, `l.aba.w1` **47**, `l.aba.1` **13,697**, **`l.aba.c1` 545**, `l.aba.2` **1,472** — and 545 is **7** from the invalidated branch and **499** from the other. A second run of the whole payload, after a reset, reads **544**. Both cross-checks hold so the cell was not void before its answer was read. 🟢 **The one-pass correction is what made the cell able to answer**: at 32 passes the separation is 1.34× and a true invalidation would have read as warm. `SPEC.md` `CPU-70`; `CPU-45` is closed from the timing side with no `CCTL` primitive |
...
~~4096~~ **8,192** iterations is a coarse version of that route, and whether it
is good enough is exactly what `t-ovh` decides.

🟢🟢 **2026-09-14: it was good enough, and it split them.** The three
readings had been in `bench/` since 2026-08-29 and no file had interpreted
them. `docs/rlx-cache-and-cp0.md` § ⓑ-2 is where that happened; the short
form is that a warm KSEG0 load costs **0.07 ns** where a cold one costs
**138.43 ns** and an uncached one **104.54 ns**, which leaves *no
read-allocate* unsustainable and *the alias is snooped* standing. ⚠️ The
transfer from an 8,192-load walk to `c-A`'s single load is **推**, and the
proxy is still not a real bus master.
...
| **writing `TC1DATA`/`TCCNR`** to get an 18.79 s counter instead of a 10 ms one | it would be strictly better as an instrument, but it is a **register write** where every window in this payload already fits inside 3 ms of a 9.9998 ms wrap. Recorded as the upgrade path for `R5-0`, declined here. 🟢 **2026-09-03, seating 11: the path was taken and it worked** — `rtl819x-timer` wrote `TC1DATA = 0x80000000` (2²⁷ ≪ 4) and set `TCCNR` bits 29/28, and TC1 ran for **703 s** across a full wrap with `tc0_undisturbed=1` throughout. ⚠️ **Two things about the 18.79 s in this row, and neither is a correction to the decision it records.** ① It is **2²⁸** / 14,286,057; the driver chose **2²⁷**, which is 9.395 s at that same rate — so the row's figure and the driver's period are different bit widths and must not be read as the same quantity. ② Both are the **loader's** clock rate. 量 2026-09-03: under Linux `CDBR` reads `03E80000` (divisor 1000, not 14), so at that rate 2²⁷ is **671.07 s** and 2²⁸ would be 1,342.14 s. 🟢 **The reasoning in this row is untouched** — `probe3` runs bare metal at the loader prompt, where 9.9998 ms and 18.79 s are both correct — and the numbers are annotated here rather than rewritten because this row records a decision that was made with them. `notes/timer-driver.md` § 8.1 |
...
`0xC0DE0300|i` and `0xD1CE0300|i`, 讀 `probe3.c:2051-2075 (v1 = rlx_call0_primed)`) — so the destination
...
`probe3.c:1793-1806 (THE RETAINED BITMAP)` states the design: *"THE RETAINED BITMAP. One sweep point
