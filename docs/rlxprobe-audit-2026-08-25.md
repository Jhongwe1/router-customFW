...
| **§5** `V_NOSTORE` conflates two states | Did not arise. Cell 1 against cell 5 measured that **a cached store to a line the D-cache does not hold reaches memory unaided**, and the four-way table the audit put in the sheet is what made it decidable. 🔄 **2026-08-26: this row said *"measured the D-cache write-through"*, and that is one reading of it.** Both cells stored to a line the cache did not hold, so write-through and write-back-without-write-allocate are indistinguishable here — `notes/cache-model.md` § *Correction, 2026-08-26*. **The audit's own point is untouched**: the four-way table is what kept the case decidable either way |
