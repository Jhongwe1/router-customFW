...
**Release: [`v0.4`](https://github.com/Jhongwe1/router-customFW/releases/tag/v0.4) · what it does not establish: [`docs/KNOWN-ISSUES.md`](docs/KNOWN-ISSUES.md).**
...
| **What that claim does *not* say** | *"zero flash bytes written"*. The evidence reaches **1,024 bytes of a 4,194,304-byte part — 0.0244 %** — four windows read back across two power cycles. 🔄 **2026-09-08 (seating 16): the clause that used to end this sentence — *"and no instrument here can establish more than that"* — is now false, and the correction is not a bigger number.** A second instrument exists and it answers a **different question**. The bracket asks *did these bytes change **across this seating***; `R5-5`'s driver digests flash on the device and asks *are these bytes identical to the **2026-08-16 reference***. Neither subsumes the other: the digest cannot see a write during a seating that restored the original value, and the bracket cannot see anything outside its four windows. **So 0.0244 % is unchanged and still answers its own question.** What the new instrument found is a **difference**: 28,672 bytes proven identical to the reference — including the **whole loader region, all 24,576 bytes**, where the bracket had sampled 256 — and **4,096 bytes at `[0x9000,0xA000)`, exactly one erase sector, proven different**. 🔴 **That makes *"zero flash bytes written"* worse than unmeasured: it is now known false for the DEVICE over some interval, with the write unattributed and not attributable by this seating.** What rlxfw can still say is narrower and is measured: its own driver counted zero writes, and the loader region is intact over all of it. `SPEC.md` `FLS-26`. 🔄 **2026-09-02: this read *512 bytes* and that was two errors in one clause.** 512 is `H601`'s own coverage (512 of its 8,192 = **6.25 %** exactly — written unrounded because 6.25 is a half-way value and `round()` and prose disagree about it), and pairing it with the whole part's 4,194,304 understates the real figure by half; the whole-part number has been 1,024 since seating 8 on 2026-08-31, and seating 9 re-read four windows on 2026-09-01 without moving it. Caught by re-deriving it against `CLAUDE.md` and `PROGRESS.md`, not by any checker — `CNT-1`'s class. `PROGRESS.md`'s `R0` row carries the wording and why it changed 🔄🔄 **2026-09-09 (seating 18): both of the coverage numbers above are superseded, and the differing region GREW.** `rtl819x-spi`'s two-level `map` ran in full: `map 0` says 31 of 32 groups are identical to the 2026-08-16 reference, and `map 1 0` splits the remaining group into 32 units of 4,096 — **30 same, 2 DIFFER**. So **proven identical goes 28,672 → 4,177,920 bytes (99.61 %)**, **proven different goes 4,096 → 8,192 bytes**, and **undetermined goes 4,153,344 → 8,192 bytes — which is exactly `H601`, skipped by rule inside the driver.** 🔴 The second differing unit is `[0xD000,0xE000)` and **nothing had ever seen it**, because a prefix digest finds the first difference and nothing past it — so *"exactly one erase sector"* above was never checked, only never contradicted. 🟢 **And the attribution bracket closed byte-identical**: `cmp` on the two seatings' `map 0` captures is identical, and the vendor firmware ran twice between them, so those two runs wrote nothing to the 4,186,112 bytes it covers. ⚠️ **The 0.0244 % bracket is still unchanged and still answers its own question**, and *"zero flash bytes written"* is exactly as unsayable as this row already says. |
...
([`SPEC.md`](SPEC.md) `CLK-25`) — 🟢 **on 2026-09-06 the clockevent half took over the system tick on the silicon, three times on three cold boots, and the proof is causal rather than correlational: change its reload and the kernel's clock changes with it, 1× to 10×, every ratio landing to four decimals** ([`SPEC.md`](SPEC.md) `CLK-26`) — 🟢 **and on the same day, one seating later, version 4.1 arms it FROM INSIDE THE KERNEL: eleven boots whose system tick is this driver's before `/sbin/init` is executed, proved by two boot marks' order in one capture rather than by any `/proc` field** ([`SPEC.md`](SPEC.md) `CLK-27`) — 🔴 **and version 4.0 refused its own handover twice first, because its pre-check window spanned the vendor's driver initialisation, over which TC1 loses 11 of 585 interrupts against a 1 % tolerance. The tolerance was not widened; the window was moved** ([`SPEC.md`](SPEC.md) `IRQ-13`) — 🟢 **and on 2026-09-06 a second driver joined it, `drivers/gpio/rtl819x-gpio.c`: a `struct gpio_chip` on the SoC's `PABCD` port that reads the reset button through gpiolib and writes nothing at all — ten boots, `gpiochip_add()` returning 0 every time, ten boot captures byte-identical, and the write guard refusing on the silicon at two layers with two different errnos** ([`SPEC.md`](SPEC.md) `REG-35`) — 🔴 **and chasing a bit that moved when it should not produced the seating's largest result, which was not about my driver at all: the vendor's reset-button path, read out of this image's own compiled code and then confirmed on the die — a one-second timer, SIGTERM to PID 1 at a 2–4 s hold, and the factory-default flag at 5 s, whose only readers in this image are two `/proc` handlers** ([`SPEC.md`](SPEC.md) `FW-40`) — written without reading anyone else's
...
place. 🟢 **2026-09-16: the gate closed and § 11 is the adjudication of all four
registrations — one confirmed, one refuted, one confirmed by a route it did not
name, and one cashed.** § 8 itself is **not edited**, which is the only thing
that makes the previous sentence checkable; the three sentences elsewhere on the
page that expired when the gate closed are struck in place rather than removed,
and § 11.4 is the receipt. Every other document here was written after its evidence, and a document
