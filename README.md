...
| **Not measured** | Everything about the **core itself**. The instruction set, the pipeline hazards and the CP0 registers are read out of binaries and vendor source; nothing of mine has yet executed a single instruction on this silicon. That is gate `R1`, and it runs bare metal because Linux emulates the two rows the toolchain decision rests on |
