...
| **Not measured** | 🔄 **The cache geometry, and the two routes to it are now measured shut rather than untried**: `Config` reads zero so there is no `Config1`, and the R3000 sizing walk needs cache isolation that this part does not implement, so it can only return its own *no answer* value. What is left is an eviction walk that needs no isolation. **The pipeline hazards**, which need a controlled loop and a timing harness. And 🔴 **what `0x0000CD01` is the part number of** — the value is measured, the mapping to a Lexra model number is not, so **`RLX5281` stays unwritable, and so does `RLX4181`** |
