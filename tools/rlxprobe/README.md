...
| **`probe3`** | `R1h` / `R3-9`. 🔴 **This file did not mention it at all until 2026-09-13** -- it arrived after the README was last written, and the payloads table stayed at three while the directory had four. Sixteen cells across seven groups: the I-cache geometry by eviction walk, `CCTL` behaviour, CP3 reachability with `CU3` held, the memory-mapped SPI window at two strides, and the two controls every later payload quotes -- `x-ri` (a reserved encoding) and four `cache` op values | ✅ **RAN TWICE.** 2026-08-29 (`bench/2026-08-30/QJ.log`, 29,088 bytes, `1a0725c0…`) and 2026-08-31 (`bench/2026-08-31c/K-J.log`, 31,536 bytes, `fc7b21d4…`, 718 result words). The I-cache is **16 KiB, 16-byte lines, two-way**, the second route to two-way being a SHAPE rather than a count -- ten `{k, k+256}` pairs with no singleton. `x-ri` trapped with `ExcCode 0x0A` and all four `cache` ops retired **in the same capture**, which is what makes a zero elsewhere in that table a reading rather than a dead handler |
| **`probe4`** | `R1a`. **The instruction sweep**: 75 encodings, one row each, three verdicts -- traps with its `ExcCode` / does not trap and is RIGHT / 🔴 does not trap and is WRONG -- plus `RAN` for a row with no computable answer, which `plan:716-719` says is not evidence of support. The cells are GENERATED by `tools/isapay.py` from `tools/isa-payload.tsv` and `isapay emit --check` fails if one byte of `cells4.S` was not. **It compares nothing**: the expected constants are not in the image, so there is nothing there for a wrong expectation to be silently right against | Written, gated (`hazlint`: 487 loads, 0 violations), verified three ways over all 75 rows, and it **runs to its end marker under qemu** -- `qemu/2026-09-13/probe4.txt`, 72 of 75 pre-registered qemu predictions hit, all four verdict cells reached, every control fired. 🟢 **量 2026-09-14, seating 21**: it ran on the device. `bench/2026-09-14/C1-P4j.log` / `C1-P5j.log` |
| **`probe5`** | `R1b`. **The hazard ladder**: 24 rows over 10 families, and a row here is a SEQUENCE -- a producer, `dist` `nop`s, a consumer -- so the reading is the DISTANCE at which the consumer stops seeing a stale value. Every row carries TWO expected constants and they are different kinds of claim: the interlocked one is the ISA's (讀), the exposed one is a hypothesis about this implementation (推), because MIPS-I says reading too early is UNPREDICTABLE -- so a row that reads NEITHER is a larger finding than one that reads the stale value. Generated by `tools/hazpay.py` from `tools/isa-hazard.tsv`. 🔴 **Its gate is `tools/hazdecl.py` and it runs the other way round**: the cells contain, on purpose, the hazards `tools/hazlint` exists to refuse, so the UNMODIFIED gate scans the WHOLE image and its output is adjudicated in both directions -- and `hazlint` exiting 0 is a BUILD FAILURE, because a hazard payload with no hazards has had them compiled away | Written, gated (266 loads / 7 violations / 0 unresolved / 4 survey sites / 14 padded rungs in neither channel / 24 distances all `4*(dist+1)` in the symbol table), and it **runs to its end marker under qemu** -- `qemu/2026-09-13/probe5.txt`, **24 of 24 rows LOCK**, which is C4 and is the vacuous answer recorded FIRST. 🟢 **量 2026-09-14, seating 21**: it ran on the device. `bench/2026-09-14/C1-P4j.log` / `C1-P5j.log` |
| **`probe6`** | `R1f` and `R2c`'s silicon row. **Ten compilers, one C fragment**: `frag.c` -- `void rlxf(volatile unsigned *d, volatile const unsigned *s) { *d = *s; }` -- is compiled once per compiled row of `tools/isa-toolchain.tsv`, by four toolchains at six `-march` values, and all ten objects are linked into ONE image whose framework is built by none of them. Each is called with a known sentinel in `$v0` one instruction before its load, so the two answers are constants: the loaded value (LOCK) or the register's prior value (OPEN). 🔴 **The fragment was chosen by refutation**: the obvious one had its load delay slot filled with `jr ra` in all nine configurations tried, which is `PROGRESS.md:122`'s risk column arriving as a measurement. 🔴 **Its gate is `tools/tcpay.py gate` and it runs the other way round**, like probe5's -- the UNMODIFIED `hazlint` over the WHOLE image, adjudicated in both directions, plus `G8`, which reads the instruction words out of the ELF and does not use `hazlint` at all. 🔴 **It cannot be built into this directory's `build/`**: the rsdk drivers are 32-bit and cannot stat a file on DrvFs (`SPEC.md` `TC-56`), so `BUILD=` must name a real Linux filesystem and the generated makefile refuses otherwise | Written, gated (`hazlint` rc 1, 139 loads, 0 unresolved, 7 violations at 7 declared `nopad` sites, 5 `pad` sites in neither direction), and it **runs to its end marker under qemu** -- `qemu/2026-09-14/probe6.txt`, **12 of 12 rows LOCK**, which is the forced anti-control and is the vacuous answer recorded FIRST. **Not yet run on the device**: its card is frozen at `bench/2026-09-14b/PREDICTIONS-B20-block19.md` and unrun |

🔴 **AND THE SAME SHAPE AS probe3's ROW, ONE SEGMENT LATER.** That row says this file did not mention `probe3` at all until 2026-09-13 -- *the payloads table stayed at three while the directory had four*. `probe5` landed on 2026-09-13 and this table was at four while the directory had five, and it was the closeout audit's *which owner file was not told* pass that found it, not a checker. There is still no checker.
